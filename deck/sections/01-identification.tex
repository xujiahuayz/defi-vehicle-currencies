...
% VISUAL-FUNCTION: observable vehicle use | authentic explorer screenshot plus native decoded component | show why the transaction headline is not itself an economic route
\input{../output/exhibits/route_replay_deck_values.tex}
\begin{frame}{The transaction headline omits the intermediary route}
\begin{columns}[T,totalwidth=\textwidth]
\begin{column}{0.57\textwidth}
\evidencekicker{What the explorer shows}
\vspace{0.08cm}
\centering
\includegraphics[width=\linewidth,height=3.55cm,keepaspectratio,trim=250 150 20 170,clip]{assets/observed_route_blockscout.png}
\par\vspace{0.04cm}
\raggedright
{\scriptsize The interface summarizes a PYUSD--USDe swap executed through 0x.\par}
\end{column}
\begin{column}{0.40\textwidth}
\evidencekicker{What the ordered logs reveal}
\vspace{0.18cm}
\begin{tikzpicture}[x=1cm,y=1cm,scale=0.88,transform shape]
\node[nodepoint] (a) at (0.35,2.75) {USDC};
\node[nodepoint,fill=ucldark,text=white,draw=ucldark] (k) at (2.20,2.75) {USDT};
\node[nodepoint] (b) at (4.05,2.75) {USDe};
\draw[route,draw=ucldark] (a)--(k);
\draw[route,draw=ucldark] (k)--(b);
\node[font=\scriptsize\bfseries,text=ucldark,align=center] at (1.28,3.45) {Fluid};
\node[font=\scriptsize\bfseries,text=ucldark,align=center] at (3.13,3.45) {Uniswap V4};
\node[font=\scriptsize,text=muted,align=center] at (1.28,1.85) {\RouteReplayInputAmount{} USDC\\$\rightarrow$ \RouteReplayVehicleAmount{} USDT};
\node[font=\scriptsize,text=muted,align=center] at (3.13,1.85) {\RouteReplayVehicleAmount{} USDT\\$\rightarrow$ \RouteReplayOutputAmount{} USDe};
\node[softbox,text width=4.25cm,minimum height=1.05cm,align=left,anchor=north] at (2.20,1.08) {\scriptsize Within the same transaction, one connected component routes \textbf{\$\RouteReplayValue} through USDT across two venues.};
\end{tikzpicture}
\vspace{-0.10cm}
...
\end{columns}
\vspace{0.05cm}
{\scriptsize Ordered pool events identify the route component and its intermediary. The example shows what we observe; it does not show that the route was optimal or representative.\par}
\end{frame}
