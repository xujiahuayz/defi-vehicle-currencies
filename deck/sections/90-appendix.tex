...
% VISUAL-FUNCTION: sample attrition | exclusion diagram pending certified funnel | show round-trip and value-support restrictions
...
% VISUAL-FUNCTION: protocol-specific state requirements | list pending protocol-variable matrix | prevent false comparability across AMM families
\begin{frame}{A5. AMM designs require different state variables}
\begin{description}
\item[Constant product] Reserves, fee, and invariant.
\item[Concentrated liquidity] Active liquidity, price, tick, and initialised-tick changes.
\item[StableSwap families] Balances, amplification, rates, and oracle state.
\item[Weighted pools] Vault balances, weights, scaling, and rate providers.
\item[V4 singleton] Pool key, tick state, and fee for standard pools without custom hooks.
\end{description}
\vspace{0.25cm}
...
% VISUAL-FUNCTION: liquidity quantities | definitions pending labelled pool cross-section | prevent capital, inventory, depth, and quote quality from collapsing
...
% VISUAL-FUNCTION: opportunity set versus conditional choice | concepts pending multiplex network | separate feasible paths from selected routes
