...
\evidencekicker{All route lengths; monthly atomic-pair graphs}
\vspace{0.18cm}
...
\begin{frame}{A16. Ultimate-pair composition contains a mechanical component}
\vspace{-0.10cm}
\begin{center}
\resizebox{0.90\textwidth}{!}{\input{assets/vehicle-transition-pair-decomposition.tex}}
\end{center}
\vspace{-0.08cm}
{\large Changes within fixed ultimate pairs are \WethCountWithinFullChange{} overall and \WethCountWithinNoEndpointChange{} after removing WETH endpoints.\par}
\decknote{WETH cannot be both an ultimate-pair endpoint and the intermediary. Exact two-atomic-trade native-WETH-versus-stablecoin routes on common month-days; fixed-pair estimates also absorb month-day and route scope.}
\end{frame}
...
\decknote{Exact additive decomposition on 181 matched dates in 2024 and 2026. Endpoint direction describes the ultimate pair; the vehicle remains the intermediary token.}
\end{frame}
...
\end{columns}
\vspace{0.68cm}
{\large Local two-atomic-pair depth and broader network position both matter.\par}
\decknote{Five vehicle currencies compared within the same ultimate-pair--date--route-scope set. Depth is prior-calendar deposited capital on the weaker atomic pair. The regression absorbs set and vehicle effects and clusters by ultimate pair and date.}
\end{frame}
...
\end{column}
\end{columns}
\decknote{\BridgePoolMarginEvents{} first-use events. A newly active pool has positive prior-calendar capital on day zero and none on day -7; continuing pools are positive on both dates. Difference inference clusters by ultimate pair and adoption date.}
\end{frame}
