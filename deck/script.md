...
### Q&A crib: the payment rails named on the implications slides

- **Nexus** (BIS Innovation Hub Singapore Centre + partner central banks): multilateral interlinking of domestic instant payment systems (PayNow, UPI, PromptPay, DuitNow, InstaPay); each system connects once and reaches all the others, payments in under 60 seconds. No shared currency: every cross-currency payment is converted by a competing FX Provider inside the scheme, so FXP access rules are exactly the lever the slide points at. Status: blueprint completed 2024; founding central banks India, Malaysia, Philippines, Singapore, Thailand incorporated Nexus Global Payments in Singapore in 2025; implementation work through 2026, go-live targeted around 2027. MAS is a founder, so this is the local example.
- **Rialto** (BIS Innovation Hub, Eurosystem and Singapore Centres): experiment on FX conversion and settlement for instant cross-border payments using wholesale CBDC; in its design, when two currencies have no direct market the payment routes through a vehicle currency. Purely experimental, not live.
- **mBridge**: shared multi-CBDC platform for wholesale cross-border settlement among the central banks of China, Hong Kong, Thailand, the UAE, and Saudi Arabia (joined 2024). Reached minimum viable product in 2024; the BIS handed it to the member central banks in late 2024 and it now runs without BIS. Real transactions settle on it, especially China-UAE, with reported cumulative volume in the tens of billions of dollars by 2026. Members settle directly in local currencies, an explicit attempt not to inherit the dollar bridge; membership decides which corridors get a direct leg. If geopolitics comes up: it is often read as the non-G7 bloc platform, with Project Agora on the G7 side; our point is design-neutral, access rules allocate the vehicle role whoever runs the rail.
