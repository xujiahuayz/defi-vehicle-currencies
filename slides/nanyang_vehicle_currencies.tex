...
\vspace{0.8em}
\begin{itemize}
  \item WETH and stablecoins are vehicle-currency candidates in the data.
  \item V3 and V4 are architecture changes that reveal how market design affects vehicle use and settlement.
\end{itemize}
\end{frame}
...
\end{itemize}
\source{Authors' calculations from reconstructed Ethereum DEX route data.}
\end{frame}
...
\bottomrule
\end{tabular}
\source{Authors' calculations from reconstructed route-cost and liquidity data.}
\end{frame}
...
\begin{itemize}
  ...
  \item The route-cost evidence therefore separates feasibility, thin-direct protection, and common-support price improvement.
\end{itemize}
\end{frame}
...
\vspace{0.6em}
\begin{itemize}
  \item The empirical object is persistence in liquidity and route use.
  \item The estimates are predictive evidence rather than a liquidity-shock design.
\end{itemize}
\end{frame}
...
\end{itemize}
\source{Authors' calculations from reconstructed route and liquidity data.}
\end{frame}

\begin{frame}{Figure: vehicle shares and liquidity}
\centering
\includegraphics[width=0.86\textwidth,height=0.72\textheight,keepaspectratio]{lp_concentration_timeseries.pdf}
\source{Authors' calculations from reconstructed route and liquidity data.}
\end{frame}
...
\end{itemize}
\source{Authors' calculations from common-support stress event design.}
\end{frame}

\begin{frame}{Figure: common-support stress rotation}
\centering
\includegraphics[width=0.88\textwidth,height=0.72\textheight,keepaspectratio]{stress_common_support.pdf}
\source{Authors' calculations from common-support stress event design.}
\end{frame}
...
\begin{itemize}
  ...
  \item Main empirical claim: no-direct/WETH-available cases fall.
  \item The V3 evidence is strongest for direct-route opportunity, not for every architecture outcome.
\end{itemize}
\end{frame}
...
\end{itemize}
\source{Authors' calculations from pair-level route opportunity panel.}
\end{frame}
...
\end{itemize}
\source{Authors' calculations from matched V3/V4 route units and transaction receipts.}
\end{frame}

\begin{frame}{Figure: settlement-transfer incidence}
\centering
\includegraphics[width=0.88\textwidth,height=0.72\textheight,keepaspectratio]{v4_settlement_transfer_incidence.pdf}
\source{Authors' calculations from matched V3/V4 route units and transaction receipts.}
\end{frame}
...
\begin{itemize}
  ...
  \item The vehicle-linked component strengthens where vehicle use is high and during stress.
\end{itemize}
\source{Authors' calculations from reconstructed vehicle-linked liquidity panel.}
\end{frame}
...
\begin{frame}{Scope of the evidence}
\begin{itemize}
  \item Route-cost estimates are scoped to quoteable venues; venue coverage is reported separately.
  \item Liquidity estimates measure persistence and predictability rather than shock-based causal effects.
  \item Stress rotation is an impact-window result, with hourly and weekly diagnostics used to study duration.
  \item V4 evidence measures intermediary-token transfer incidence and matched route use while separating route intermediation from physical settlement.
  \item Curve and Fluid are included in realized vehicle-use measurement and excluded from executable-depth quote panels.
\end{itemize}
\end{frame}
