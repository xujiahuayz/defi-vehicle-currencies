% ============================================================================
% nbc_2026_full.tex
% FULL merged deck for the invited talk at the Nanyang Blockchain Conference
% 2026 (NTU Singapore, 21--22 August 2026): core content (this file's own
% preamble/title/core sections, originally authored as nbc_2026_core.tex)
% followed by a single "Backup" divider and the full backup content
% (originally authored as the standalone nbc_2026_appendix.tex, merged in
% below). Built from the winning framing (output/nbc_pipeline/02_framings/
% framing_1.md) and the Phase-4/5 verified evidence under
% output/nbc_pipeline/04_evidence/. Do not add numbers to this file that are
% not traceable to those verified sources.
%
...
%
% MERGE NOTES (2026-08-04, compiled and QA'd in this pass):
% 1. The two source files' preambles were near-identical (both
%    \documentclass[aspectratio=169]{beamer} with the same package list and
%    plain theme); this file keeps the core deck's preamble, which is a
%    strict superset (it additionally loads tikz for the three mechanism
%    diagrams the appendix never needed). No missing packages, no
%    duplication.
% 2. The core deck's own condensed backup frames (a mini P1 headline table,
%    a short robustness paragraph, P2 sample/DiD, P2 placebo, P2
%    volatility/version, and its own References slide) were DROPPED at the
%    Backup divider rather than kept next to the appendix's fuller versions
%    of the identical material -- see the note at the "\section{Backup}"
%    marker below for why.
% 3. FACTUAL FIX: the core deck's "P1's contra-result" frame claimed the
%    D-on-L wrong sign "recurs in the low-baseline-depth subsample." The
%    appendix's own depth-split backup slide (built by checking the primary
%    p1_robustness_depth_split.csv/.md directly, not the prose synthesis)
%    found the opposite: all 8 depth-split cells match the *predicted*
%    sign, i.e. this is the one split where the contra-result reverses. The
%    bullet below has been corrected to match the appendix's verified
%    finding so the merged deck does not contradict itself between the core
%    section and its own backup.
% ============================================================================
...
\begin{frame}{P1's contra-result: one leg of the loop runs backward}
  \small
  P1 predicts deeper own liquidity should \emph{lower} DirectCostAdvantage ($D$ on $L$, $\beta<0$). The pooled estimate is significantly positive at every horizon. It survives most, but not all, of the robustness battery:
  \begin{itemize}
    ...
    \item Recurs in the pre-midpoint half of the sample; reverses (matches the predicted sign) post-midpoint.
    \item \textbf{Reverses} in the high/low baseline-depth split: both regimes match the predicted sign there (full detail in backup).
  \end{itemize}
  \vspace{0.4em}
  \begin{alertblock}{Bottom line}
    Two of four legs of the proposed loop show lead-lag support ($S$ on $L$ robust, $L$ on $S$ partial); one is null; one runs opposite to the theory in most but not all splits and stays unresolved.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Recap and contribution}
  \begin{itemize}
    \item Liquidity depth and routing volume move together in the direction P1 predicts ($S$ on $L$ robust across every check), with one contra-result left unresolved ($D$ on $L$, wrong-signed across most, but not all, robustness splits).
...
% ============================================================================
% BACKUP
% Divider + appendix content merged in from nbc_2026_appendix.tex (companion
% backup deck). NOTE (merge decision, 2026-08-04): the core deck's own
% original condensed backup frames (P1 headline mini-table, a short
% robustness-detail paragraph, P2 sample/DiD, P2 placebo, P2
% volatility/version, and a References slide) have been DROPPED here rather
% than kept alongside the appendix's fuller versions of the same material --
% keeping both would have put two different-looking tables of the *same*
% P1/P2 numbers back-to-back and two slightly different References slides
% in one deck. The appendix version is a strict superset (more SE/p columns
% on the P1 headline table; dedicated slides for each robustness split
% instead of one paragraph; a Q&A coverage map and design-notes section the
% core-only backup never had), so it wins. See the appendix's own file
% header for its sourcing/verification method.
% ============================================================================
\section{Backup}
...
\begin{frame}
  ...
  {\small
  Q\&A coverage map \quad $\vert$ \quad P1 backup: full headline table and $D$-on-$L$ robustness detail \\
  P2 backup: placebo dates, pretrend, volatility split, version construction \\
  Design notes: data provenance, cost construction, why V2$\to$V3 \quad $\vert$ \quad References
  }
  \vfill
\end{frame}
...
\begin{frame}{Q\&A coverage map (1/2): mechanics and data}
  \footnotesize
  \begin{center}
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{5.6cm}>{\raggedright\arraybackslash}p{5.9cm}@{}}
    \toprule
