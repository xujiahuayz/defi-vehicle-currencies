...
from scripts.tabulate.render_v4_flash_gap_interactions import (
    render_v4_flash_gap_interactions,
)
...
def test_v4_flash_gap_interaction_table_scales_units() -> None:
    ...
    assert "Candidate-linked TVL (log pts)" in rendered
...
def test_v4_flash_gap_interaction_table_rejects_incomplete_grid() -> None:
    rows = _complete_rows()[:-1]
    with pytest.raises(ValueError, match="incomplete"):
        render_v4_flash_gap_interactions(pd.DataFrame(rows))
