...
def _event(
    *,
    ...
    return {
        ...
        "owner": f"0xowner{tx[-1]}",
        "sender": f"0xsender{tx[-1]}",
        "pool": {
            "id": f"0xpool{tx[-1]}",
            "token0": {"id": token0, "symbol": "WETH" if token0 == WETH_ADDRESS else "USDC"},
...
def test_v3_lp_flow_loader_prices_candidate_sides_and_screens_outliers(
    tmp_path: Path,
) -> None:
    ...
    assert weth["v3_gross_lp_flow_usd_screened"] == pytest.approx(5_000.0)
    assert weth["v3_add_lp_flow_usd_screened"] == pytest.approx(4_000.0)
    assert weth["v3_remove_lp_flow_usd_screened"] == pytest.approx(1_000.0)
    assert weth["v3_narrow_flow_usd_screened"] == pytest.approx(4_000.0)
    assert weth["v3_wide_flow_usd_screened"] == pytest.approx(1_000.0)
    assert usdc["v3_gross_lp_flow_usd_screened"] == pytest.approx(1_000.0)
    assert usdc["v3_lp_flow_above_screen_assignments"] == 1
    assert support["above_screen_assignments"] == 1


def test_v3_lp_flow_run_writes_panel_and_support(tmp_path: Path) -> None:
    event_dir = tmp_path / "events"
    event_dir.mkdir()
    price_path = tmp_path / "prices.parquet"
    candidate_day = tmp_path / "candidate_day.parquet"
    fee_panel = tmp_path / "fees.parquet"
    ...
    ).to_parquet(fee_panel, index=False)
    with gzip.open(event_dir / "uniswap_v3_mints_20250105.jsonl.gz", "wt") as handle:
        handle.write(
            json.dumps(
                _event(
    ...
    assert (
        run(
            ...
            fee_panel_path=fee_panel,
            price_path=price_path,
        )
        == 0
    )
    assert pd.read_parquet(output)["v3_gross_lp_flow_usd_screened"].sum() == pytest.approx(
        2_002.0
    )
    assert "v3_lp_flow_support" in support.read_text()
