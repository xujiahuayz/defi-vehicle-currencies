...
def test_current_release_lease_blocks_pointer_switch_until_reader_exits_twenty_times(tmp_path: Path) -> None:
    pointer = tmp_path / "release" / "current.json"
    for trial in range(20):
        ...
        with current_artifact_release(selected):
            ...
            assert json.loads(selected.artifacts["rows"].read_text()) == {"value": trial}
