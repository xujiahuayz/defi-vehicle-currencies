...
conditions that made the audit possible at all, and both were violated in the draft it
read: a numeric claim has to carry a provenance comment, and the artefact that comment
names has to exist on disk. The missing-artefact case was real, `realised_dominance_reweighting.jsonl`
was cited by section 3 and absent, so the number behind the paper's headline arithmetic
...
# A provenance comment names a path under output/, docs/, scripts/, src/ or data/.
...
class PaperProvenanceTests(unittest.TestCase):
    ...
    def test_numeric_paragraphs_carry_a_provenance_comment(self) -> None:
        missing: list[str] = []
        for path in self.sources():
            ...
            for lineno, prose, comment in paragraphs(path.read_text(encoding="utf-8")):
                ...
                if not nums:
                    continue
                if not PROVENANCE_PATH.search(comment):
