...
Section~\ref{sec:dominance} puts an asset in the vehicle role at moments when routing through it was the more expensive way to trade, on 27.2\% of realised multi-leg routing over four consecutively priced days. A frequency of that kind is a statement about states, and the question this paper adds is about time. Volume-cost feedback can support multiple vehicle configurations under particular payment structures \citep{Krugman1980VehicleCurrencies}. In a distinct random-matching model, fixed fundamentals can support different commodity media because acceptance is coordinated by extrinsic beliefs \citep{KiyotakiWright1989MoneyMedium}. Direct switching costs can keep users in an established currency in a one-shot choice \citep{DowdGreenaway1993CurrencyCompetition}. None supplies the cost-conditioned transition duration measured here. The separation measured here is a speed, the rate at which the role changes hands on a pair whose incumbent has lost its cost advantage.
