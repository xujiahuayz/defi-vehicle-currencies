...
For three exchange families, full-day Ethereum-log reconciliations cover 206 venue-days across 74 sampled dates. We compare every provider swap with logs queried independently from the chain. The comparison can add a chain event absent from the provider data, remove a provider event absent from the chain, or replace its payload or log position. We then rerun the same route construction on the 50 sampled dates with at least one swap correction. Table~\ref{tab:app:route-validation} reports both the event match and its consequences for the economic route. Available indexed records omit 35 historical auxiliary venue-days; the table note reports their coverage and the log-order collision that sets the comparison's boundary. The measured scope is the sampled dates and the connected pool sequence observed in each transaction.
...
Table~\ref{tab:bridge-pool-margins} separates newly active pools from capital growth in pools already supporting the route. Newly active pools hold \BridgePoolMarginNewCapitalShare{} of stablecoin weak-leg capital on the first-use day; \BridgePoolMarginContinuingCapitalShare{} lies in pools active one week earlier. Most observed growth therefore comes from deepening pools that already exist, with a smaller pool-activation margin.
