...
\]
the 30 basis point fee both venues charge, applied on chain as the integer ratio above.\footnote{The SushiSwap v2 atomic-pair contract implements the same adjustment; see \href{https://github.com/sushiswap/v2-core/blob/master/contracts/UniswapV2Pair.sol}{\texttt{UniswapV2Pair.sol}}.}
...
\bottomrule
\end{tabular}
\exhibitnote{Absolute quote error in percent of realised output. The sample contains 4,218 realised Uniswap v4 swaps on three dates across 13 pools and nine atomic pairs, at static fee tiers 0, 7, 8, 100, 500, 10,000 and 20,000. Each quote uses liquidity events ordered strictly before the swap and the price state left by the preceding swap in the same pool.}
\end{table}
