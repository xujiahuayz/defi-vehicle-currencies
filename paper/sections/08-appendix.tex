...
Uniswap v3 and v4 distribute reserves across price ranges, so a pool has no single actual reserve pair and a quote has to walk the price axis \citep{AdamsZinsmeisterRobinson2021UniswapV3}. Inside one range the pool behaves as a constant-product pool with virtual reserves $\widetilde R_{0,p,e^-}$ and $\widetilde R_{1,p,e^-}$. Liquidity $L_{p,e^-}$ and the token1-per-token0 pool price $\rho_{p,e^-}$ satisfy
\[
\widetilde R_{0,p,e^-}=\frac{L_{p,e^-}}{\sqrt{\rho_{p,e^-}}},
\qquad
\widetilde R_{1,p,e^-}=L_{p,e^-}\sqrt{\rho_{p,e^-}},
\]
so the token amounts exchanged over a price move from $\sqrt{\rho_a}$ to $\sqrt{\rho_b}$ are
\[
\Delta_0=L_{p,e^-}\left(\frac{1}{\sqrt{\rho_a}}-\frac{1}{\sqrt{\rho_b}}\right),
\qquad
\Delta_1=L_{p,e^-}\left(\sqrt{\rho_b}-\sqrt{\rho_a}\right).
\]
...
\[
\sqrt{\rho'}=\sqrt{\rho}+\frac{\Delta_1}{L_{p,e^-}},
\qquad
\sqrt{\rho'}=\frac{L_{p,e^-}\sqrt{\rho}}{L_{p,e^-}+\Delta_0\sqrt{\rho}}.
\]
