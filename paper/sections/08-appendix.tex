...
The counterfactual comparison requires an exact-input quote from each pool's state and the proposed input amount. Because venue families use different invariants and arithmetic, we validate their implementations separately. For selected executed swaps, we reconstruct the state immediately beforehand and compare predicted with realised output. Curve's amplification coefficient and Balancer's weight ratio are unavailable in the historical subgraph records; the component exercises estimate them from one set of realised trades and evaluate quotes on separate trades. These exercises establish feasibility for particular pool families and dates. The cost comparison additionally requires complete pre-transaction states for every included family and date.
...
Every counterfactual quote uses the pool state immediately before the trade, whereas some historical records store balances only at the end of an hour or day. We recover the earlier state by ordering every balance-changing event within the period and unwinding the sequence from the recorded closing balance. The exercises below validate that procedure on selected constant-product and weighted-pool observations. Complete reconstruction across all families and dates is required before the cost comparison can be estimated.
...
Omitting a venue can change the best direct route, the best intermediated route, or both. We therefore begin the coverage audit with eight venue series, including Uniswap v1 as a separate architecture sample. Volume is reconstructed from individual swaps using the smaller dollar value of the two legs. This avoids double counting in two venue series and removes implausible oracle values in thin pools: one Curve pool reports a \$456m day whose swaps sum to \$8, and a constant-product pair with \$255 of reserves reports \$1.36bn. Physical bounds on volume relative to reserves exclude between 0.10\% and 0.24\% of pool-days per year. The route-cost opportunity set subsequently excludes Uniswap v1 because its forced-routing architecture is studied separately; the eight-series coverage audit is broader than the priced opportunity set.
