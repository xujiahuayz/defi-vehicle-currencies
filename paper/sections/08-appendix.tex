...
The counterfactual panel prices at the close of an hour, whereas the route-selection state is latent and may precede the observed settlement block. Section~\ref{sec:executability:timing} first measures how much prices move between the pre-settlement and hour-end states. Because both alternatives are priced at the same state, their ranking changes only with relative price movement. A closed triangle isolates that relative component.
...
\]
is positive exactly when the direct pool returns more of $b$ per unit of $a$ than the two-leg route does, in the zero-size limit where the marginal price is the pool price. The sign of $m$ gives the direct-versus-vehicle ranking at that limit.
...
\begin{table}[t]
\centering
\caption{Route-ranking changes between pre-event and hour-end states, by day}
...
\begin{table}[t]
\centering
\caption{Route-ranking disagreement, conditional}
...
Panel C shows that restricting the sample to large residuals does not remove the timing problem: the disagreement rate remains between 34.1\% and 44.5\% across the pre-event residual buckets and is 36.2\% in the top bucket against 41.0\% in the bottom one.
% output/exhibits/block_vs_hour_conditional.jsonl, gap_at_own_event rows over 189,622 opportunity snapshots
A triangular-arbitrage residual can oscillate around zero within a block, so even a large observed value may be short lived. Panel B provides an independent timing check: rankings disagree less often when an opportunity occurs close to the hour end and more often when prices have longer to move. Figure~\ref{fig:app:boundary} shows this relationship.

\begin{figure}[tbp]
\centering
\caption{Route-ranking disagreement by time remaining to the hour boundary}
\label{fig:app:boundary}
\begin{tikzpicture}
\begin{axis}[ybar, width=0.85\textwidth, height=6.4cm, bar width=0.9cm, symbolic x coords={under 1,1 to 5,5 to 15,15 to 30,30 to 60}, xtick=data, xlabel={Minutes remaining to the close of the route's hour}, ylabel={Ranking disagreement, percent}, ymin=0, ymax=54, ytick={0,10,20,30,40,50}, tick align=outside, tick label style={font=\footnotesize}, label style={font=\footnotesize}, nodes near coords, nodes near coords style={font=\footnotesize}, point meta=explicit symbolic, grid=major, grid style={black!10}, axis line style={black!70}, enlarge x limits=0.12]
...
Panel A shows why the timing error varies across routes. A two-leg route pays two pool fees where a direct route pays one. This relatively fixed wedge moves the comparison boundary away from the region in which market prices fluctuate, so route rankings are more stable when the wedge is larger.
