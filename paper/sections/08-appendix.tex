...
In the Pooled row of Table~\ref{tab:app:support}, five per cent of input reserves sits between the 90th percentile of 3.29\% and the 99th of 14.86\%; the same cutoff lies between those columns on seven of the eight date rows. Figure~\ref{fig:app:support} shows the daily distributions. This calculation is a depth diagnostic. The empirical rule remains a 5\% own-price-impact cutoff computed separately under each pool family's pricing function.
...
The exclusions are concentrated in routes involving the native asset. In Panel B of Table~\ref{tab:app:curveleg}, the excluded share of native-leg volume exceeds the stable-leg share by at least 33 percentage points in every sample year and by 54 points in the 2023 row. Tricrypto pools pairing WBTC and WETH with a stablecoin account for \$818m, or 44.6\%, of the \$1.83bn excluded, including seven of the twelve largest excluded pool-days. These pools use CryptoSwap, so the missing coverage is concentrated where Curve supplies depth between ETH and stablecoins.
