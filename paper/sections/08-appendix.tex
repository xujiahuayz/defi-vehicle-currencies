...
This section reports evidence used directly to interpret the main estimates: entry-activity restrictions, differences across venue pricing families, the composition of capital growth before first use, and subsequent comovement between route use and capital. Pool states and first-use dates are equilibrium outcomes, so the estimates describe how route use and liquidity line up.

\subsection{Persistence among pairs with greater entry activity}

Table~\ref{tab:app:entry-persistence-robustness} repeats the post-entry stablecoin-share regressions after requiring at least five or ten routes on the entry day. The estimates remain close to one-for-one at both horizons, so the main persistence result is also present among entrants with a more precisely measured initial allocation.
