...
Omitting one sampled date preserves the full-graph dollar-weighted ordering in both periods. Omitting one venue gives wider rank ranges: WETH spans ranks three to four in 2026 H1, while USDC and USDT each span ranks one to two (Table~\ref{tab:app:network-centrality-robustness}, panel B). Removing legs between stablecoins places WETH first, USDC second, and USDT third in 2026 H1 (panel C). The dollar-weighted stablecoin lead therefore comes partly from the stablecoin core; WETH remains the leading connector outside those core legs. The issuer split in Appendix Table~\ref{tab:app:endpoint-direction}, panel B, keeps this scope dependence beside the stable-to-stable endpoint composition that generates it.
