...
\section{Common support for hypothetical routes}
\label{app:support}

Each quoter above was accepted against swaps that executed, so its validation sample necessarily contains pools deep enough to serve the observed trade. A hypothetical route can demand much more output relative to a pool's depth and thereby move the calculation beyond any region in which quote error has been measured. I retain a hypothetical leg only when its own price impact lies within a range represented among the realised validation trades. The cutoff is calibrated from that population.
...
\footnotesize\raggedright \emph{Notes.} The statistic is a leg's input amount as a fraction of the input-side reserve in the pool it would traverse. The 5\% cutoff falls between the 90th and the 99th percentile in the pooled distribution and on seven of the eight days separately. The exception is 2020-05-05, which contains two validated swaps at the same size-to-depth ratio. The sample contains 932,270 validated swaps on eight days from 2020-05-05 to 2025-09-23.
\end{minipage}
\end{figure}
...
This restriction removes economically relevant observations, and the body reports that loss of support. It cannot measure quote error for retained hypothetical routes because those routes were never executed and have no realised output for comparison. Section~\ref{sec:executability:arbitrage} instead develops an indirect check based on same-block arbitrage.
...
One venue in the top eight is excluded outright. Sushiswap's concentrated-liquidity deployment presents as a balance-and-weights venue by field inventory, and its weight vector takes exactly one value across every sampled day, the default a two-token schema writes when no weights exist, so a weighted-product quoter has nothing to read. Correct weights would price the wrong invariant because the venue's reserves sit across ticks. The available schema also omits \texttt{sqrtPriceX96}, in-range liquidity and tick spacing, leaving the tick quoter without its state variables. The exclusion costs almost nothing: the venue is 0.016\% of the five priced venues' volume pooled, peaking at 0.176\% in 2026, and across fourteen sampled days only 4.1\% of its volume sits on token pairs no priced venue hosts that day, never exceeding \$37,609 of unique-pair volume on any sampled day.
