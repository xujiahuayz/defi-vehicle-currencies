...
This restriction removes economically relevant observations, and the body reports that loss of support. It cannot measure quote error for retained hypothetical routes because those routes were never executed and have no realised output for comparison. Same-block capture motivates a possible indirect check, but the required transaction-level closing-cycle test has not yet been completed.
...
The realised-route validation matches each leg of a two-leg V3 route to its pool and transaction log, reads both pools strictly before the transaction's first V3 swap, and compares that state with the hour-end state used by the panel. Across 12,931 routes on three dates, the median absolute movement in the two-leg marginal rate is 54.3 basis points; 62.8\% move by more than 30 basis points and 36.4\% by more than 100. The median is 140.3 basis points on 2022-06-15, 52.1 on 2024-06-15, and 31.7 on 2025-09-14. These dates reveal heterogeneity but do not establish a trend or an aggregator effect.
% output/exhibits/realised_route_timing_validation.jsonl, 12,931 routes on three dates
