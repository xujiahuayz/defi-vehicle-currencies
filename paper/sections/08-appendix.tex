...
\]
Both outputs are computed from one reconstructed state. This is the property the realised-trade comparison lacks and the reason for constructing the counterfactual.
...
\[
A_p n^{n}\sum_jR_{j,p,t}+D_p
=A_pD_pn^{n}+\frac{D_p^{n+1}}{n^{n}\prod_jR_{j,p,t}},
\]
...
\]
Only the ratio $w_{i,p}/w_{o,p}$ enters, so an equal-weight Balancer pool and a constant-product pair with the same reserves quote identically and every departure is the weight ratio doing work. Quotes are refused past the vault's cap of 30\% of the input token's reserve, because a swap beyond that reverts on chain and a number returned there would be a price the panel could not have traded.
% src/ddvc/pricing/weighted.py, MAX_IN_RATIO at 30% of the input balance

The weight ratio and the swap fee are both reported by the subgraph and both can be stale, since the schema serves them at the head block while reserves come from a historical snapshot. Liquidity-bootstrapping pools move weights on a schedule and any weighted pool's owner can reset the fee, and the fee case showed up in validation before it was diagnosed: read parameters reproduced 2024 trades to $10^{-9}$ percent and 2022 trades only to 0.19\%, a fee-sized gap, since the fee enters through $\varphi_p=1-f_p$ and a fee wrong by a basis point moves a quote by about a basis point. Both parameters are therefore identified the way Curve's $A_p$ is, with reading tried first and each fit introducing exactly one free scalar over at least four observations on the pair.
...
\bottomrule
\end{tabular}
\exhibitnote{A pool serves the \emph{native} leg when it contains ETH or a staking derivative, the \emph{stable} leg when every token is stable including baskets and interest-bearing wrappers, and the \emph{other volatile} leg otherwise. The sample contains 28 measured Curve pool-days from 11 February 2020 through 28 April 2026. Pool leg is assigned by a token registry pass followed by a symbol pass over stable derivatives because the registry alone labels metapools pairing a stablecoin against the three-token base-pool claim as volatile pairs.}
\end{table}
