...
\end{aligned}
\end{equation}
The venue-specific forms of $Q_p$ appear in Section~\ref{sec:setting:pricing}. Pool invariants take token quantities as arguments, which is why the dollar notional $q$ and the token input $\Delta_i$ must remain distinct. Equation~\eqref{eq:route} describes an executed route and, once market state has been reconstructed, a hypothetical route.

Observed routes provide an automated-market counterpart to the classical volume--cost mechanism. Greater use of a vehicle expands trading in its two bilateral markets and can lower the cost of using it again \citep{Krugman1980VehicleCurrencies}. In \citet{KiyotakiWright1989MoneyMedium}, traders accept an asset for indirect exchange when its marketability, which depends on other traders' strategies and inventories, compensates for its storage cost; \citet{DowdGreenaway1993CurrencyCompetition} emphasise the fixed costs of learning, accounting, and quoting in a new currency. For automated markets, liquidity placement, venue access, and the paths available to routing software provide the corresponding sources of coordination. Vehicle dominance measures one outcome of that coordination: intermediary use along an observed trading path. The intermediary role is one of several ways in which a currency can become dominant. Invoicing concerns the currency named in trade contracts \citep{GopinathEtAl2020DCP,AmitiItskhokiKonings2022Dominant,Mukhin2022InternationalPriceSystem}; funding concerns the denomination of liabilities \citep{GopinathStein2021Making,ErenMalamud2022DominantDebt}; and dealer turnover records trading activity \citep{Somogyi2026DollarDominanceFX}. Each can reinforce intermediary use, while Equation~\eqref{eq:route} identifies the asset used between the currency sold and the currency bought. We observe its settlement block; because the router may have formed its quote earlier, Section~\ref{sec:executability} incorporates this timing difference into the counterfactual price design.
...
We begin with the competition between native and stable intermediaries. Let $\mathcal R^{(2),NS}_t$ denote exact two-leg routes on day $t$ whose intermediary belongs to either group, and let $\mathcal R^{(2),NS,V}_t$ retain the subset with supported dollar values. The equally weighted and value-weighted group shares are
\begin{equation}
\label{eq:dominance}
S^{N}_{g,t} \;=\;
...
\label{eq:excess-use}
\end{equation}
A positive $\Delta^N_{k,t}$ means that the asset's share of intermediary episodes exceeds its share of endpoint appearances; $X^N_{k,t}$ expresses the same comparison as a ratio. The value-weighted measures replace counts with supported dollar values while retaining the same currency perimeter. Because excess use compares each currency's intermediary and endpoint roles, its denominator differs from the native-or-stable denominator in Equation~\eqref{eq:dominance}. The latter remains the measure of the headline rotation.
