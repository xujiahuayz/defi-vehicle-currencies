...
These routes provide an automated-market setting for the classical volume--cost mechanism. Greater use of a vehicle expands trading in the two markets joined through it; deeper markets can then lower the cost of using the same vehicle again \citep{Krugman1980VehicleCurrencies}. In \citet{KiyotakiWright1989MoneyMedium}, traders accept an asset for indirect exchange when its marketability compensates for its storage cost. \citet{DowdGreenaway1993CurrencyCompetition} emphasise the fixed costs of learning, accounting, and quoting in a new currency. On decentralised exchanges, liquidity placement, venue access, and the routes available to software supply corresponding sources of coordination. Observed paths reveal the intermediary that results from this process.
...
A transaction on January 10, 2026 illustrates both the information in the logs and its boundary. A public explorer describes a 460,000 PYUSD--USDe swap executed through 0x.\footnote{Transaction \ethtx{0xbda1d07f17c503b5a9c8117c5d1383472006af8a9e22aa4e5e30804b9bf67ad9}{0xbda1d07f\ldots 9bf67ad9}.} The user's instruction begins with PYUSD, but the exchange-pool sequence begins when 459,929.81 USDC enters Fluid. Fluid returns 460,331.05 USDT, which then enters Uniswap v4 and produces 460,104.28 USDe. The route observed in the pools is therefore USDC--USDT--USDe, with USDT as the vehicle. We do not assign the earlier PYUSD--USDC conversion to an exchange pool because it occurs outside this connected sequence. Applying the same rule throughout the sample preserves the distinction between the user's instruction and the trading path we can observe.
% output/exhibits/route_replay.json; data/unified/20260110.parquet; Blockscout transaction page retrieved 2026-08-14

The observed sequence supports two measurements. It identifies the intermediary used in the transaction, which is the basis of our dominance measure. With the state of every relevant pool immediately before execution, it can also support a same-size comparison between the realised path and available alternatives. We introduce notation for the observed route here and develop the price comparison in Section~\ref{sec:executability}.
...
\end{aligned}
\end{equation}
The venue-specific forms of $Q_p$ appear in Section~\ref{sec:setting:pricing}. Pool invariants take token quantities as arguments, which is why the dollar notional $q$ and the token input $\Delta_i$ remain distinct. Equation~\eqref{eq:route} describes an executed route and, once market state has been reconstructed, a hypothetical route.

Intermediary use is one of several currency roles. Invoicing concerns the currency named in trade contracts \citep{GopinathEtAl2020DCP,AmitiItskhokiKonings2022Dominant,Mukhin2022InternationalPriceSystem}; funding concerns the denomination of liabilities \citep{GopinathStein2021Making,ErenMalamud2022DominantDebt}; and dealer turnover records trading activity \citep{Somogyi2026DollarDominanceFX}. These roles can reinforce one another, but the path in Equation~\eqref{eq:route} identifies a narrower object: the asset used between the currency sold and the currency bought. We observe the block in which the route settles. Because a router may have formed its quote earlier, the price comparison allows for that timing difference.

\subsection{Measuring vehicle dominance}
\label{sec:setting:definitions}

An exact two-leg route passes through one pool on either side of a unique intermediary. This gives the primary measure a simple unit: each route contributes one intermediary observation. Longer routes, which can use several intermediaries, enter separate measures of network reach. For route $r$, let $\mathcal{M}(r)$ denote the set of assets between the source and destination. We record the intermediary status of asset $k$ as
\begin{equation}
...
To study execution costs, we compare two counterfactual benchmarks at the state immediately before transaction $e$ and at a common dollar input notional $q$. \citet{XiMoallemi2026Routing} compare realised allocations with optimal allocations across pools for a single token pair. Our object instead compares a direct path with a sequential path through an intermediary asset. The direct benchmark is the best single-hop execution between the observed endpoints. The vehicle benchmark is the best two-hop execution through intermediary $k$, allowing each leg to use its best priced venue. For the set of pools available at that state, $\mathcal P_{i,o,e^-}$, the two outputs are
\begin{equation}
\label{eq:direct}
...
\end{align}
Each $\mathcal G$ combines gas units calibrated by year, route structure, venue sequence, and intermediary identity with the transaction's effective gas price and the dollar price of the native asset. The two benchmarks use the same input and declared venue set, but interpreting their difference as evidence about route choice also requires the state seen by the router, its venue access, and any costs outside the measured perimeter. Section~\ref{sec:executability} sets out these requirements. Constant-product, concentrated-liquidity, StableSwap, and weighted-pool rules allocate liquidity differently over the price curve, so a trade-size comparison must pass the full input through the state transition of the relevant pool. Appendix~\ref{app:quoters} presents the pricing equations and component-level validation exercises, whose individual implementations reproduce selected realised swaps. A reportable cost comparison additionally requires complete pre-transaction states for every included pool family and a common audit of venue coverage.
...
\qquad \bar{\pi} = 0.05,
\end{equation}
where $\widetilde Q_{\ell}$ prices leg $\ell$ at the pool's marginal rate. Across pool families, empirical support is defined by the same 5\% own-price-impact ceiling, while protocol-specific transaction limits impose additional execution constraints. Input relative to reserves calibrates the constant-product case; own-price impact supplies a comparable rule for concentrated-liquidity, StableSwap, and weighted pools. Because executable depth in concentrated-liquidity pools depends on the active tick map, a common reserve-ratio cutoff would compare different economic objects. Appendix~\ref{app:support} reports the 932,270-swap constant-product calibration and the corresponding validation exercises for the other pool families. The current evidence is confined to these component-level exercises. A pooled counterfactual additionally requires complete pre-transaction states for every relevant pool and compliance with Equation~\eqref{eq:support} on every hypothetical leg.
% output/exhibits/quoter_support_bounds.jsonl
% scripts/run_route_cost_panel.py, MAX_INPUT_TO_RESERVE and MAX_PRICE_IMPACT

\label{sec:setting:validation}

Vehicle dominance and the counterfactual cost gap answer separate questions. Equation~\eqref{eq:dominance} measures observed intermediary allocation, whereas Equation~\eqref{eq:gap} defines the output difference between two hypothetical paths. The continuous gap $\Delta C^{D}_{i,o,k,q,e}$ measures cost in basis points. A coefficient on $D^{\mathrm{g}}$ instead measures a change in the incidence of direct-route dominance. No cost coefficient enters the route-composition results.
