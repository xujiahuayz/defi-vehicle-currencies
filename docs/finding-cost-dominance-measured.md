...
Status 2026-08-10: gross-output comparisons remain construction evidence, but every all-in incidence and magnitude below is withdrawn pending the exact transaction-receipt rebuild. The analytical route panel no longer accepts a daily gas-price join. It requires the realised transaction's `effectiveGasPrice` joined by exact transaction and block, requires the receipt block hash to match the independently fetched block header, applies that price to both alternatives, and uses that header's same-block `baseFeePerGas` as the urgency-free sensitivity. Pre-EIP-1559 observations have no base-fee sensitivity by construction. The exact `gasUsed × effectiveGasPrice` charge is admissible in wei; dollar and basis-point versions additionally require a backward as-of join to the canonical independent intraday WETH/USD observation strictly preceding the transaction. A daily WETH/USD mark is sensitivity-only.
...
The figures in this subsection are a retired path-only pilot. The active replacement validates both realised legs and their composition at a maximum absolute error of one basis point, and no pilot magnitude below enters a current claim.
