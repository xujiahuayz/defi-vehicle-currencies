...
**F6. Per-day gas and per-day gas-token price, candidate-specific and venue-specific.** The daily-price half is closed: `data/processed/daily_gas_price_graph.parquet` covers all 2,277 project days with zero unresolved dates, using every transaction in three evenly spaced full blocks and a prespecified two-block minimum. The open half is gas units per candidate and venue, not the flat 74,096-unit per-hop fallback. A Curve stableswap leg and a tick-crossing concentrated-liquidity leg do not cost the same gas as a constant-product leg, and a constant common to candidates inside a group is absorbed by the group fixed effect. The live receipt instrument conditions on year, topology, exact venue sequence and intermediary identity; its completed support distribution must determine the fallback hierarchy before F recomputes all-in cost.
