...
**The size of that threat is now measured on the verdict itself instead of on the price level, and it turns out to be a function of trade economics and not a single number.** The 86.2% figure is a LEVEL, being how far one pool's own state sits from the hour-boundary state, and a dominance verdict is a COMPARISON in which a market-wide move affects the direct pool and both vehicle legs together and cancels. `output/exhibits/block_vs_hour_verdict.jsonl` measures the comparison directly, using the sign of the triangle residual $m = \log P(A\to B) - \log P(A\to K) - \log P(K\to B)$ over 190,073 route-level observations in 371 triangles, on the seven sampled days that yielded triangles between 2021-12-25 and 2025-11-04, where token decimals cancel exactly around the closed triangle and `sqrtPriceX96` is observed on the swap event. Three results. The verdict flips 38.0% of the time at zero trade size. Restricting to large gaps does not help, at 35.6% for gaps above 100 basis points, because at zero size $m$ is a triangular-arbitrage residual that searchers keep oscillating around zero within blocks. And the fee wedge does help steeply, since a two-leg route pays two pool fees against one and that offset does not move with the market: the flip rate falls to 24.5% at a 30 basis point wedge, 15.6% at 60 and 8.9% at 100. The test validates itself on the time dimension, where the rate rises monotonically from 15.6% for routes executing within a minute of the hour close through 21.0%, 32.4% and 38.9% to 44.9% for routes 30 to 60 minutes from it, which is the profile a real timing effect must have and a bug in the test would not produce. An earlier note here quoted that first bucket as 14.3% and paired it with the last two from a different run, mixing a six-day sample with an eight-day one, which is the same defect as quoting one day's round-trip share as a corpus constant and is corrected on the full seven-day set.
