...
| The pair-by-day design cannot resolve the sign, and calling it a null asserts an absence the data cannot support | Same, 703 of 22,991 pair-day cells and 3,865 of 102,845 routes identify, standard error 0.085, minimum detectable effect near 24 percentage points | EXISTS | n/a |
| Quoting every candidate for every pair-day removes the coincidence the single-venue estimator waits on, and settles the sign | Multi-venue panel quotes all five candidates at three notionals by construction | PENDING | No |
...
| Native-type indicator | -0.049 (0.008) | -0.051 (0.008) | -0.049 (0.008) | +0.094 (0.269) | PENDING |
| Pair-day cells identifying | | | | 703 of 22,991 | PENDING |
| Routes identifying | | | | 3,865 of 102,845 | PENDING |
| Minimum detectable effect, percentage points | | | | 24 | PENDING |
| Clusters | 3,654 pairs | 3,654 | 3,654 | 158 | PENDING |
...
|---|---|---|---|---|
| Thick-market cost advantage | $C^I_{i,o,k,q,t}$ for stable candidates crosses below the native candidate's at the share-crossover date, and $\beta_K<0$ under pair-date fixed effects | $\beta_K<0$; cost crossover leads or coincides with the share crossover | PENDING | PENDING |
| Cost of holding the intermediary | The value-weighted crossover precedes the count-weighted crossover, and $\chi_\tau$ on $\mathrm{CandidateStress}_{k,t}$ is negative for the native candidate and positive for stable candidates | Value crossover earlier; $\chi_\tau$ sign flip by type | 2022-Q1 against 2026-H1, EXISTS; $\chi_\tau$ PENDING | Partly supported |
...
Status: PENDING. Figure 2 is the exhibit, and nothing in the repository currently puts cost and share on one time axis.
...
Status: partly supported. The count-value ordering EXISTS. The stress coefficients are PENDING, and the March 2023 depeg is the episode that identifies the reversal.
...
Ordered by how much narrative weight is blocked, with the specification that would produce each.
...
**F2. Figure 2, cost and share on one time axis.** Median $C^{I}_{i,o,k,q,t}$ by candidate type against $\mathrm{TypeShare}^{\theta}_t$, monthly, $q=\$10{,}000$, on common support $\mathcal C_{k,t,q}$. This is the single exhibit that converts the paper from a description of a transition into a statement about what made it, and no artefact in the repository currently joins the two panels. The transition is measured on 2,240 days of the unified layer and the cost on 186 sampled days of the v2-only counterfactual, with different venue coverage, which means the join needs the multi-venue panel from F1 and not the existing counterfactual.
...
Handoff note, 2026-08-06. F appears to be mid-delivery on this item in a parallel session: `scripts/run_vehicle_dominance_hdfe.py` and `output/exhibits/vehicle_dominance_hdfe.jsonl` are present and uncommitted, estimating `dominated ~ native | cell_id` clustered by pair with the cell defined over pair, time window and size bin, and reporting a minimum detectable effect alongside each coefficient. On the one-day window the artefact carries 177,106 identifying cells against the 703 the v2 panel could reach. G does not promote any number from it, because status EXISTS in this file requires a `docs/finding-*.md` write-up that states the sample, the filters, the sign convention and the confounds. Two points G will check when that document lands: the estimate's magnitude sits far from the pooled single-venue coefficient of -0.049 (0.008) and needs reconciling in prose, and the near-invariance of the coefficient across window lengths from 1 to 60 days needs an explanation, because a cell definition that widens by a factor of sixty while the estimate moves in the third decimal is either a robust effect or an effect the cell is not absorbing.

**F4. Registry symbols for asset-type shares.** Add $\mathcal K^{\theta}$, $\mathrm{TypeShare}^{\theta}_t$ and $\mathrm{TypeCountShare}^{\theta}_t$ to `src/ddvc/variable_registry.py` with $\theta$ over the five types in `src/ddvc/asset_types.py`, measured over the observed intermediary population and not over $\mathcal K$. The paper's lead table has no registered notation today, which breaks the registry's role as single source and breaks the rule that table shells use registry symbols.

**F5. Value-weighted type shares for the missing years.** Table 3's value-weighted block reports 2020, 2022, 2024 and 2026 in the findings document and omits 2021, 2023 and 2025. Fill them, and report the quarterly crossover dates for both weightings so the paper can state the two crossovers with dates instead of with year labels.
...
Node H has produced nothing yet. No deck, slide source, or exported artefact exists anywhere in the repository, and no narrative gap has therefore been exposed by H. Recording that plainly matters for the convergence condition, because the cluster cannot be called converged while one of its three members has never run.

What G requires from H when it does run. Each item is a test H should try to fail, and none is a specification to conform to.

**H1. Whether the lead result survives compression to one slide with no table.** The gate requires every result to be a plot or a diagram first with tables in backup only. Table 3 is a ten-column, seven-row exhibit and figure 1 is its intended replacement. If the two crossovers, both weightings and the non-monotonicity cannot be read off one plot, section 3 is carrying more structure than its claim needs and G shortens it.

**H2. Whether the horse race is legible without the paper.** Section 5's four accounts have to work as running labels, which is what they do in the exemplar this pattern comes from. If a slide cannot name the four accounts and the one fact that separates each, the accounts are not distinct enough and G merges or drops one.

**H3. Which claim a reader asks about first.** The order H finds natural is evidence about whether section 3 or section 4 should lead, and Java's reserved call on which results lead is still open. Section 4.1 of the workflow says the transition leads. Section 8's item 3 gives a different inclination, and its numbering refers to a candidate list that section 4.1 replaced, which makes it stale where it reads as contradictory. G has built to section 4.1's ordering and flags the discrepancy for Java.

**H4. Whether the V1 null belongs in the opening or in the horse race.** G currently places it in both, as a motivating fact in the introduction and as the evidence favouring account 5.4. If a deck can carry it only once, G needs to know which position a reader finds more useful, because the two readings pull in opposite directions.

**H5. Whether anything in the deck makes a claim the paper does not.** The convergence condition requires every slide to map to a G section and add no claim of its own. Any slide that needs a number G has not asked F for is a demand back into this file.
