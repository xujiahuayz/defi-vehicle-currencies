...
| Dominance incidence differs sharply by intermediary type, at native 23.7%, stable 45.4% and imported 61.4% | Same, and the ordering is what the thick-network definition of the native type implies, so it is reported as description and not as a finding, per 2.5 | EXISTS at four days; PENDING (rebuild) | Partly |
...
What section 4 may say today is limited by the screen in `docs/node-e-screen-persistence.md`, and the limit is stated here so no downstream document overstates it. The persistence result is admissible as a cross-sectional fact about share differentials, with the block-timing threat of 6.2 named as open. It does not license the word hysteresis, which needs the displacement arm, and it does not license a causal reading, which needs the objective-mismatch bound of section 6.2. The one screen the result already passes is the definitional one: persistence is measured against the same vehicle's share when not dominated and not against zero, so a story in which routing shares are merely sticky predicts no differential at all, and native moving 68.6% to 39.4% is a large differential.
...
| A router quotes at a block and the panel prices at an hour boundary, so a route cheapest at quote time and dominated by the hour's close records as dominated when the router could not have avoided it | `docs/node-e-screen-persistence.md` threat 1 | EXISTS as a stated threat, OPEN as a defect. This is the highest-value remaining piece of empirical work in the project |
| The magnitude of that staleness is the same order as the effect, at a median 0.345% of intra-day state movement on the deepest USDC/WETH pool with a worst hour of 1.04%, against route-cost differences of tens of basis points | Same, and `docs/research-workflow.md` section 4.0 | EXISTS |
| Re-pricing each realised route at its own block closes the threat, and the data supports it because every swap carries `blockNumber` and the tick venues carry `sqrtPriceX96` per swap | Same | PENDING. Specification: `measure_realised_dominance.py` matching on block instead of hour, with the hour-to-block wedge reported |
...
**F1. Re-price realised routes at their own block instead of at the hour boundary.** `docs/node-e-screen-persistence.md` names this as the open threat to the persistence result and as the highest-value remaining piece of empirical work, and G agrees. A smart-order router quotes at a block and executes in the next; the panel prices at the end of an hour; intra-day state movement on the deepest USDC/WETH pool runs at a median 0.345% with a worst hour of 1.04%, against route-cost differences of tens of basis points. So the staleness is the same order as the effect and a route the router chose while it was cheapest can record as dominated. Specification: match each realised route to pool state at its own `blockNumber`, using `sqrtPriceX96` per swap on the tick venues and the swap sequence to roll v2-family reserves back, and report the hour-to-block wedge as its own exhibit so the size of the correction is visible. This does not threaten the dominance frequency, which is a statement about a state, and it does threaten every retention ratio in table 4.
