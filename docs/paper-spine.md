...
**The size of that threat is measured on the verdict at opportunity times, but the instrument does not yet validate realised routes.** The 86.2% figure is a level, being how far one pool's state sits from the hour-boundary state, and a dominance verdict is a comparison in which a common market move can cancel. `output/exhibits/block_vs_hour_verdict.jsonl` uses the sign of the marginal triangle residual $m = \log P(A\to B) - \log P(A\to K) - \log P(K\to B)$ on 371 triangles over 189,622 eligible V3 direct-pool opportunity snapshots. The corrected implementation orders pool states strictly before each target in block-log order; the old block-only `bisect_right` lookup included the target swap's own post-state and the conditional denominator admitted observations from triangles that failed the sample screen. Under the corrected contract, the hour-boundary verdict flips 38.89% at zero wedge, 25.34% at a 30-basis-point wedge, 16.15% at 60 and 9.43% at 100. The time profile rises from 22.49% within one minute of the hour boundary to 45.55% at 30 to 60 minutes; the old first bucket of 15.56% was mechanically low because it contained the target post-state. These are opportunity snapshots at direct-pool swap times, not realised multi-leg routes, and the instrument omits depth. It bounds the timing threat and cannot reinstate the persistence claim. F1 still requires state immediately before each realised route.
