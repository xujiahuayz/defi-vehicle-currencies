...
**F6. Per-day gas and per-day gas-token price, candidate-specific and venue-specific.** The daily-price half is reopened because its retired full-block sampler selected blocks from one active DEX stream. Rebuild it from exact chain-wide UTC-day boundaries and fixed interior block quantiles, require all requested blocks to resolve, and retain canonical block identity plus the complete gas-price vector in one compressed raw evidence object. The other open half is gas units per candidate and venue, not the flat 74,096-unit per-hop fallback. A Curve stableswap leg and a tick-crossing concentrated-liquidity leg do not cost the same gas as a constant-product leg, and a constant common to candidates inside a group is absorbed by the group fixed effect. The live receipt instrument conditions on year, topology, exact venue sequence and intermediary identity; both rebuilt support distributions must pass before F recomputes all-in cost.
