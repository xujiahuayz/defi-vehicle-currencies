...
Current edge: `C <-> K`, with `C -> E -> I` active on the routing-efficiency branch. The replacement vehicle-extent definition, dated backing attack, venue-technology rival and single-versus-cross-venue split are implemented. The within-vehicle and alternative-path diagnostics are implemented and have failed their first timing interpretation check as intended; the full corrected run and own-block validation remain open. Historical v4 state and the triplet-period dominance design remain red. Any defect found here routes back to C/E; no defect is patched only in prose.
