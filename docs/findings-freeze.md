...
| Routing efficiency / aggregator era | design now separates expanding venue/pool reach, realised-to-best search efficiency and vehicle-liquidity economics, and treats sender labels as partial. The same-hour forced/chosen matcher is implemented with exact transaction identity and nearest log notional; monthly efficiency measures, the full match and own-block validation are not yet executed |
...
Current edge: `C <-> K`, with a new `C -> E -> I` routing-efficiency branch. The replacement vehicle-extent definition and dated backing attack are implemented; forced-versus-chosen, venue and search-efficiency attacks remain open. Historical v4 state and the triplet-period dominance design remain red. Any defect found here routes back to C/E; no defect is patched only in prose.
