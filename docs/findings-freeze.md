...
| Routing efficiency / aggregator era | opportunity-set integration is measured on 358,027,668 clean route units: cross-venue economic multi-leg routing rises from 1.4% to 60.6% by count and 15.4% to 89.4% by value, while economic multi-leg incidence stays between 14.3% and 20.4% of all routes. Across 42,974,290 canonical intermediary episodes, the 2024 to 2026 native-to-stable count transition occurs inside both single- and cross-venue strata, with daily HAC changes of +26.9 and +33.3 percentage points, so migration into integrated routes cannot explain it alone. Value migration is concentrated in single-venue routes and is absent cross-venue. The same-hour forced/chosen matcher and a narrower within-vehicle search diagnostic are implemented. The latter uses exact two-leg execution amounts, log-notional interpolation and a frontier restricted to the route's observed venue reach. A smoke run on the stale panel produced only 16/772, 565/6,331 and 288/18,065 comparable routes on sampled 2020, 2022 and 2024 days, with median shortfalls of -0.80%, +0.02% and -0.14%. Negative values show that end-of-hour state movement remains mixed into the comparison; these are engineering diagnostics, not findings. The full corrected match, alternative-path frontier and own-block validation remain open |
...
Current edge: `C <-> K`, with `C -> E -> I` active on the routing-efficiency branch. The replacement vehicle-extent definition, dated backing attack, venue-technology rival and single-versus-cross-venue split are implemented. The within-vehicle search diagnostic is implemented and has failed its first timing interpretation check as intended; the alternative-path frontier, full corrected run and own-block validation remain open. Historical v4 state and the triplet-period dominance design remain red. Any defect found here routes back to C/E; no defect is patched only in prose.
