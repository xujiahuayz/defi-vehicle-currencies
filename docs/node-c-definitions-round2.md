...
**0.2 The original 53-file cache contained three false or unreadable records, including one load-bearing source.** At the time of this audit, the Kiyotaki-Wright file was a University of Chicago Press catalogue and the Ho-Stoll and Glosten-Milgrom scans had no OCR body. The corpus repair replaced the false Kiyotaki-Wright artifact, replaced Ho-Stoll with the exact published article and its three-page mathematical-appendix companion, and later replaced the August 1983 Glosten-Milgrom precursor with the exact March 1985 JFE article. The incident remains evidence that inventory alone cannot certify a literature read; the live source-set and full-text gates now enforce exact version, readable content and companion coverage.
...
**What the literature does.** Krugman supports a binary direct-versus-indirect regime boundary. He does not define this project's continuous signed route-cost gap, and neither object is a measure of vehicle dominance.
...
Krugman's `D`, which he calls the clockwisdom, is the product of the three posted exchange rates and therefore a deviation from triangular exchange-rate parity. A direction-specific net return comparison between direct and two-leg exchange also contains the transaction-cost retention terms, and in partial-indirect equilibrium `D` adjusts to make the marginal route indifferent. It is not this project's route-level basis-point gap. The binary partial-versus-total regime is a conceptual precedent; the continuous matched-quote gap is this project's measured extension.
...
**Do they match. The binary cost form has a conceptual ancestor in Krugman's regime boundary; the continuous gap and the old dominance name do not.** The route is the correct primitive for asking whether an executed path left output on the table at its own pre-trade state. A pair-candidate-period cell is the correct aggregation only for a persistence or transition estimand. The matched sample's 1.9% coverage and its inversion toward large stable-intermediated routes remain selection problems, but Somogyi's triplet unit does not solve them by relabeling the cost gap.
...
The top-level cut is stablecoin against non-stablecoin. Makarov and Schoar 2022 section 3.2: "Non-stablecoins constitute a large and diverse group", isolating first "coins that have no other function than being a cryptocurrency, either used for transaction purposes or as a store value... positioned as the new 'gold' — a digital store of value." Gorton and Zhang impose a narrower legal-institutional dichotomy between unbacked "fiat cryptocurrencies" and stablecoins backed by actual cash and safe assets. They expressly exclude algorithmic designs. This is not the project's stable/non-stable taxonomy, and "fiat cryptocurrency" is idiosyncratic vocabulary the paper does not adopt.

The second cut, and the one the corpus spends the most pages on, is **backing regime inside the stable category**. Catalini, de Gortari and Shah 2022 split stablecoins into fiat-backed, crypto-asset-backed, and those "backed partially or fully by their own investment token [which] only rely on their own algorithms and smart contracts", and state the consequence: "unlike stablecoins backed by fiat assets or cryptocurrencies, the true solvency of an algorithmic coin is linked to the public's confidence in the coin, allowing death spirals to materialize even in unstressed conditions." Lyons and Viswanath-Natraj 2023 build their whole result on that line: "in contrast to dollar-backed stablecoins, there is no clear arbitrage mechanism to restore prices when TerraUSD is priced at a discount." Four corpus papers exist principally because backing regimes differ in behaviour: Uhlig 2022, Liu, Makarov and Schoar 2023, Anadu et al 2023 and Lyons and Viswanath-Natraj 2023. Anadu et al.'s empirical groups are episode-specific, mix jurisdiction with backing, place hybrid designs in the algorithmic bucket and document DAI's changing stablecoin and real-world-asset exposure. They motivate dated exposure regimes and state-contingent safety, not a permanent crypto-collateral label through 2026.

Gorton and Zhang define no-questions-asked acceptance as par acceptance without value due diligence and present it as a condition for a reliable medium of exchange. Here it motivates a backing-and-acceptance mechanism or validation test; it neither defines vehicle status nor measures dominance.
...
`staked_native` remains separate as a project specification because it carries the native asset's exposure in a different instrument. The claim that redemption queues make it fail no-questions-asked acceptance is a hypothesis requiring a liquid-staking source, not a result in Gorton and Zhang.
...
Mukhin 2022 supplies the succession object in the same shape. His Figure 3, "Transition from One Vehicle Currency to Another", is not a concentration index; it is an **ordering of which trade flows switch first**, numbered 1 to 3: US-to-small-economies first, then small economies with one another, then UK-to-small-economies. Dowd and Greenaway 1993 supply a narrower adoption comparison: under equal network-independent fundamentals, `N2 < N1` makes currency 1 preferable when choosing which existing currency should become common. It is not a general succession threshold. Their one-shot outcomes illustrate excess inertia or excess momentum, with no transition path.
...
The original corpus defects are now resolved: Kiyotaki and Wright is the correct article, Ho and Stoll is the published article plus its essential mathematical-appendix companion, the Glosten and Milgrom working-paper precursor has been replaced by the exact published JFE article, and Flandreau and Jobst is present. The methodological conclusion survives the repair: any earlier sweep over the defective cache could have produced false nulls, so active claims must rest on the live source-set and full-text audit instead of the historical inventory.
