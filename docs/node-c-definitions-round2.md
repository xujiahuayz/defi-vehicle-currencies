...
**0.2 The original 53-file cache contained three false or unreadable records, including one load-bearing source.** At the time of this audit, the Kiyotaki-Wright file was a University of Chicago Press catalogue and the Ho-Stoll and Glosten-Milgrom scans had no OCR body. The corpus repair replaced the false Kiyotaki-Wright artifact, OCRed the scans, replaced Ho-Stoll with the exact published article, and preserved the three-page mathematical appendix that the final article delegates to its working-paper precursor. The incident remains evidence that inventory alone cannot certify a literature read; the live source-set and full-text gates now enforce exact version, readable content and companion coverage.
...
The original corpus defects are now resolved: Kiyotaki and Wright is the correct article, Ho and Stoll is the published article plus its essential mathematical-appendix companion, Glosten and Milgrom is OCR-readable, and Flandreau and Jobst is present. The methodological conclusion survives the repair: any earlier sweep over the defective cache could have produced false nulls, so active claims must rest on the live source-set and full-text audit instead of the historical inventory.
