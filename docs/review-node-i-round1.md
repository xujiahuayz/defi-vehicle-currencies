...
Not one of those frictions exists for the object this project measures. A direct pool and a two-leg route through a vehicle sit on the same chain, in the same block, at the same state, and the cycle between them is capturable inside one atomic transaction with a flash loan and no capital at risk. This repository also supplies the proof that the capacity to take it is present: `docs/deck-outline.md` A16 and `docs/router-identification-feasibility.md` record round-trip routes at 12.7% of multi-leg routes by count and 21.7% by value on the median of 79 sampled days, rising to 25.9% and 91.3% on 2025-12-06, the worst, which the project excludes as atomic arbitrage or wash trading. A market where a fifth of multi-leg value is atomic arbitrage on a typical day, and never less than 7.4% on any day sampled, does not leave a 2,459-basis-point cycle standing for an hour. An earlier version of this paragraph quoted the worst day's 90.5% as though it were the typical figure, which overstated the argument; the objection survives the correction because what it needs is continuous arbitrage presence, and the floor across days establishes that better than the peak does.
