...
| 19 | Prose grid | State variables required by each AMM family | Replace with a protocol-by-state-variable matrix using marks for reserves, ticks, amplification, weights, rates, fees and hooks. |
...
The empirical block now uses three different visual forms because it asks three different questions: quarterly lines for the aggregate path, candidate dumbbells for who moved, and interaction slopes for whether the change remains inside venue-reach strata. The mechanism network, V1 imposed-versus-optional network and exact-state reconstruction sequence also use appropriate forms. Remaining visual debt is concentrated in the appendix, where several slides still use prose grids or abstract diagrams. The highest-value remaining recoveries are the four-architecture network evolution, an authentic transaction trace and, conditional on current evidence, the venue-composition area strip. Retired numeric graphics remain retired even when their visual design was strong.
