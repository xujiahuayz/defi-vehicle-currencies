...
| 12 | Pair-composition waterfall | Whether the aggregate transition occurs within continuing endpoint pairs or through changes in pair support and activity weights | Current. One native waterfall shows the exact four-term 2024--2026 count accounting. The near-zero within-pair term remains at its true scale; single- and cross-venue totals appear as a restrained scope check. The slide does not attribute the reallocation to architecture, demand or search. |
...
The empirical block now uses three different visual forms because it asks three different questions: annual paths for when leadership moved, candidate dumbbells for which stablecoin moved, and a waterfall for whether the aggregate shift occurred within continuing endpoint pairs or through changes in the cross-section of pairs carrying vehicle routes. The mechanism network, authentic transaction replay, V1-to-V4 architecture strip, exact-state reconstruction sequence and protocol-by-state matrix also use appropriate forms. Remaining visual debt is concentrated in the appendix, where several slides still use prose grids or abstract diagrams. The highest-value remaining recovery is, conditional on current evidence, the venue-composition area strip. Retired numeric graphics remain retired even when their visual design was strong.
...
7. Reveal the non-monotone annual leadership path, with count and value separated.
8. Decompose the transition into USDC and USDT.
9. Show that the aggregate rotation comes from the changing cross-section and weights of endpoint pairs, not a common within-pair switch.
...
Every animation ends in a complete static composition. The PDF contains the final annotated transaction still, the four-panel architecture strip, the fully accumulated dominance measures and the final interaction plots. The local route reveal remains native HTML/CSS/JS rather than a rendered video; if that native companion cannot be maintained, the static PDF stands alone. Each visual is generated from an admitted exhibit or a verified institutional source; hard-coded numeric redraws are not permitted. The compiled PDF remains the portable review artefact.
