...
| 4 | Authentic transaction trace | Observable cross-venue vehicle use in one executed route | Current for the execution object. The static PDF frame and local HTML/MP4 replay share one manifest from an admitted Studio-current partition. They show ordered legs, venues, amounts and value. A direct alternative is deliberately absent until exact pre-trade state supports that separate comparison. |
...
| 10 | Quarterly line panels | Rotation by count and value through time | Current. The count and common-support-value paths show native, stable, staked-native, imported and other separately. The 2022 value lead, 2023 to 2024 reversal and renewed lead from 2025-Q1 remain visible; the slide states that calendar quarter describes timing rather than treatment. |
...
| 12 | Integration-regime slopes | Whether the transition is stronger on cross-venue routes | Current. Single- and cross-venue stable shares are shown from 2024 to 2026 for count and common-support value. The slide reports the selected-strata interaction descriptively and does not assign a causal effect to integration. |
...
The empirical block now uses three different visual forms because it asks three different questions: quarterly lines for the aggregate path, candidate dumbbells for who moved, and interaction slopes for whether the change remains inside venue-reach strata. The mechanism network, authentic transaction replay, V1-to-V4 architecture strip, exact-state reconstruction sequence and protocol-by-state matrix also use appropriate forms. Remaining visual debt is concentrated in the appendix, where several slides still use prose grids or abstract diagrams. The highest-value remaining recovery is, conditional on current evidence, the venue-composition area strip. Retired numeric graphics remain retired even when their visual design was strong.
...
1. Open with the question and ask the audience to predict the dominant mechanism.
2. Present one real trade and ask whether they expect direct execution or an intermediary.
3. Reveal the executed cross-venue route through a short local replay.
4. Generalize from one route to continuous vehicle-dominance measures.
5. Morph V1's imposed hub into the optional V2, V3 and V4 networks.
6. Establish the sample and the four mechanisms.
7. Reveal the quarterly transition path.
8. Decompose the transition into USDC and USDT.
...
Every animation ends in a complete static composition. The PDF contains the final annotated transaction still, the four-panel architecture strip, the fully accumulated dominance measures and the final interaction plots. A screen recording may accompany the local route replay, but the poster frame must communicate the full result on its own. Each visual is generated from an admitted exhibit or a verified institutional source; hard-coded numeric redraws are not permitted. The compiled PDF remains the portable review artefact, while any HTML or video is a generated live companion from the same source manifest.
