...
**The count/value wedge is partly swap style, but it is not created by route splitting.** A provisional matched-support diagnostic on 23 registered dates compares episode count and value on exactly the same within-20% route components. Stable intermediary share rises from 19.3% to 52.3% by count and from 39.9% to 80.4% by value between 2024 and 2026. The wedge remains on purely sequential routes, where the corresponding changes are 18.0% to 48.8% and 39.8% to 76.3%, and on exact two-leg routes, where they are 15.6% to 45.8% and 37.8% to 77.4%. Branched split/join routes move from 37.5% to 65.4% by count and 39.9% to 84.3% by value. Stable episodes have the heavier upper notional tail in both endpoint years: their 90th percentile is $11,760 against $5,917 for native episodes in 2024 and $3,165 against $1,255 in 2026, even though median values fall sharply for both types. The full daily morphology series independently shows branched split/join execution rising from 7.0% to 20.7% of intermediation and cross-venue execution from 25.4% to 57.2%. These are observed on-chain execution styles, not identified frontend choice or human-versus-automated behavior. Promote the decomposition only after the full matched-support panel, daily pooled value caps, exact-date inference and price-versus-quantity revaluation pass.
