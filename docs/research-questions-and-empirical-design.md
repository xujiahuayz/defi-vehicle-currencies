...
| Clark-Joseph, Ye, and Zi (2017), `ClarkJosephYeZi2017DMM` | Pure empirical; local corpus includes the published version | Layered natural-experiment template for isolating a designated supplier's marginal contribution from the loss of its venue |
...
| Krugman (1980), `Krugman1980VehicleCurrencies`; Dowd and Greenaway (1993), `DowdGreenaway1993CurrencyCompetition` | Theory | Positive adoption externalities, conditional multiplicity and inertia for RQ1 and RQ3; abrupt displacement is qualitative in Krugman, while Dowd and Greenaway model one-shot switching and no transition dynamics |
| Grossman and Miller (1988), `GrossmanMiller1988Liquidity`; Ho and Stoll (1981), `HoStoll1981OptimalDealer` | Theory | Temporary inventory absorption and finite risk-bearing for RQ2 and RQ5 |
| Brunnermeier and Pedersen (2009), `BrunnermeierPedersen2009Liquidity` | Theory | Intermediary-capital and margin mechanisms for RQ2 and RQ5 |
...
| Chen and Duffie (2021), `ChenDuffie2021Fragmentation` | Theory | Narrow comparator for the distinction between venue-level depth and aggregate allocation when orders in one asset split across disconnected exchanges; their cross-price restriction does not describe atomic cross-pool routing |
...
- **Opportunity-set efficiency:** construct the dispersion of same-state triangular residuals, direct-versus-best-route cost gaps, within-block correction, and the share of transactions executing across multiple pools or venues at the highest validated native resolution, then derive a daily series for estimation. Monthly aggregation is permitted only for a labelled descriptive plot. Keep cost-surface efficiency separate from realised router choice.
...
- **Transition test:** re-estimate vehicle extent and candidate choice within date-pair-size opportunity sets, stratified by search sophistication, and interact candidate type with the daily efficiency state. Report whether the stable-vehicle transition occurs inside comparable search regimes or is concentrated in the adoption of more integrated routing. Dynamic responses use exact 1-, 7-, 30- and 120-calendar-day links; a variable-length calendar month is never a response horizon.
...
| Anand and Venkataraman (2016) | Voluntary market makers' participation co-moves and falls when stock volume is low and order flow is one-sided, while rising with volatility; designated providers remain active when voluntary providers withdraw | **Expand:** test synchronous LP withdrawal and whether vehicle-spoke demand offsets or amplifies it without a designated provider; treat the designated-provider comparison as descriptive |
| Clark-Joseph, Ye, and Zi (2017) | During the NYSE halt, NYSE-listed stocks' quoted and effective spreads widened relative to non-NYSE stocks, while displayed NBBO depth did not; an EDGX halt did not produce an analogous group difference, and pre-halt DMM share predicted the NYSE-halt spread change | **Expand:** adapt the treatment-control event, adjacent-time placebos, second-venue falsification and supplier-exposure decomposition to estimate the marginal contribution of LP capital; define any economic indispensability threshold from this project's outcomes, not from the source |
...
| Hendershott, Jones, and Menkveld (2011) | Autoquote-induced automation narrows spreads for large stocks and raises imputed liquidity-supplier realized spread while reducing adverse-selection price impact | **Expand:** separate LP gross fee yield from LVR, gas, and net return; realized spread is an incidence benchmark, not observed profit or rent |
| Grossman and Miller (1988) | Temporary customer imbalances require finite market-maker risk-bearing and command a price for immediacy | **Mechanism boundary:** motivates temporary inventory absorption, not a capital-shock or slow-reallocation design |
| Brunnermeier and Pedersen (2009) | Funding constraints and endogenous margins make market liquidity state-dependent | **Mechanism support or refutation:** outside LP wealth shocks should move supply if AMM liquidity remains balance-sheet constrained |
...
- **Pool propagation:** estimate \(\Delta_1\ln(\mathrm{TVL}_{p,t+1})=\alpha_p+\delta_t+\beta_W\mathrm{LPWealthShock}_{p,t}+\varepsilon_{p,t+1}\); \(\beta_W>0\) shows that shocks to the incumbent provider portfolio reach aggregate pool liquidity, linking the address-level result to the Comerton-Forde market-maker balance-sheet channel.
...
|---|---|---|
| Hendershott, Jones, and Menkveld (2011) | Staggered NYSE autoquote instruments a message-based proxy for algorithmic trading; automation narrows spreads and reduces adverse-selection price impact for large stocks under an explicitly untestable exclusion and modest first-stage F statistics | **Empirical template and corroboration target:** test whether a trading technology improves execution and add network topology as an outcome; the global V3 launch is weaker than their staggered instrument, while their own design is not a modern identification gold standard |
| Chen and Duffie (2021) | In an affine/Gaussian model with disconnected local demand and imperfectly correlated exchange prices, fragmentation lowers per-venue depth while order splitting can improve aggregate allocation and the joint informativeness of the full price vector; the mechanism may disappear when orders condition on cross-venue prices | **Competing theory with a boundary:** improved aggregate execution need not imply deeper local markets, while optimized cross-venue routing is a rival integration regime outside the baseline mechanism |
...
| Grossman and Miller (1988); Ho and Stoll (1981) | Intermediation requires temporary inventory absorption and finite risk-bearing | **Establish the market-making boundary:** if netting reduces settlement movement but LP capital remains necessary, transactional intermediation is separable from physical inventory transfer but not from risk-bearing capital |
...
| Comerton-Forde et al. (2010) | Lagged market-maker income shortfalls and inventory positions predict future liquidity | **Empirical anchor:** adapt the lagged balance-sheet-state design for the V4 event, adding exogenous or predetermined LP shocks where available, and test whether net settlement attenuates sensitivity |
| Hendershott, Jones, and Menkveld (2011) | Trading technology can improve user liquidity while changing imputed liquidity-supplier revenue | **Incidence anchor:** compare user all-in cost, gross LP fee yield, LVR, and net LP return after settlement technology changes without treating realized spread as provider rent |
