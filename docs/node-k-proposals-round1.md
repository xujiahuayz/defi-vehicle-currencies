...
This project discards its most informative population. Round-trip and cyclic routes run 12.7% of multi-leg routes by count and 21.7% by value on the median day of 79 sampled, and every panel in the repository excludes them as atomic arbitrage or wash trading. They are the one class of agent in this market that provably carries no habit: an arbitrage searcher re-optimises every block, holds no frontend integration, has no brand loyalty and no attention constraint, and dies within weeks if it routes suboptimally. Its choice of cycle base asset, meaning the token the cycle opens and closes in and therefore the token the searcher must hold or borrow, is a revealed-preference ranking of transaction cost purged of everything that makes ordinary routing sticky. Turning the excluded population into the measuring instrument is the proposal.
...
**What it displaces or supports.** It supplies, without touching the counterfactual quoter, the object that the retired lagged-share persistence design and the withdrawn block-pricing persistence result were both reaching for. That matters because it is immune to the constraint that kills those: the 86.2% of routes mispriced beyond 25 basis points under hour-boundary state cannot contaminate a comparison between two populations of realised trades. It supports `docs/finding-intermediation-transition.md` by dating the transition twice on the same data and reading the gap as a magnitude.
...
Section 4.0 names LP capital allocation as a home for incumbency and nothing in this repository estimates it. `src/ddvc/analysis/lp_concentration.py` reads only `data/raw/thegraph/uniswap_v3/` and measures concentration, which section 2 already flags as the silo to stop repeating. `scripts/run_lp_repositioning_tests.py` uses aggregate mints and burns as a predictor of bridge share. Neither has an LP identity in it. But identity is there: every V3 mint and burn carries `origin`, the externally owned account that sent the transaction, and every V4 modify-liquidity record carries both `origin` and `sender`. An LP-address by pool by day panel is directly constructible over 1,884 V3 days and 546 V4 days.
...
**Identification, and the variation that carries it.** Within LP and within day, across the pools that LP already occupies. The LP fixed effect absorbs attention, gas budget, capital scale and sophistication, so the only thing moving is the cross-pool return differential. That differential is realised fees minus loss-versus-rebalancing minus gas, which `data/processed/rent_incidence_pool_month.parquet` already computes at pool level with `mean_fee`, `mean_lvr`, `mean_gas` and `mean_net`.
...
**Files and streams.** `data/raw/thegraph/uniswap_v3/uniswap_v3_mints_*` and `uniswap_v3_burns_*`; `data/raw/thegraph/uniswap_v4/uniswap_v4_modify_liquidities_*`; `data/raw/thegraph/uniswap_v2/uniswap_v2_mints_*` and `uniswap_v2_burns_*` if the key crosswalk holds; `data/processed/rent_incidence_pool_month.parquet`. Cost is the highest here, because it means an address-level panel over roughly 4,700 daily files.
...
**Three-hop and split-route counterfactual cost surfaces.** Rejected because it compounds the binding constraint. With 86.2% of two-leg routes already mispriced beyond 25 basis points under hour-boundary state, a third leg multiplies the state error, and nothing should be added on top of the quoter before the off-support error bound that node I lists as item 1 exists. Proposing more counterfactual surface area right now is proposing more of the thing under review.
