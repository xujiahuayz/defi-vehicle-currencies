...
5. **Integration across venues while indirect routing contracts.** Measured on clean canonical-endpoint routes: the cross-venue share of intermediated routing rises from 1.4% to 58.7% count-weighted and 15.4% to 89.3% value-weighted across the sample. Inside the balanced five-venue perimeter the count rise remains, 19.1% to 43.6% from 2022 to 2026, but the value share is nearly flat after 2024 and economic multi-leg incidence falls from 19.5% to 10.3%. The native-to-stable count transition from 2024 to 2026 occurs inside both single-venue and cross-venue strata, so movement into the integrated regime is not a complete composition explanation. Motivating fact and a supporting result. The series separates opportunity-set integration, the extensive indirect-routing margin and conditional vehicle composition; it does not identify routing optimisation quality or aggregator causality.
