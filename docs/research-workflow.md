...
**Validation state must be causally timed.** Reproducing a realised outcome is a valid quoter or simulator check only when every state variable contains information available immediately before that observation. Calendar-day membership is not enough, and block membership is not enough either. Replay all state-changing events by block and log index, use a strict-before lookup, and include a regression test with a state change between two observations. The V4 validator originally paired the preceding swap's price with the validation day's final liquidity map; this leaked later liquidity into earlier quotes and manufactured the only visible error in the busiest-pool sample. Interleaved replay reduced the maximum absolute error on the same 1,500 swaps to 0.000099%. The first “own-block” triangle instrument then repeated the same class of error at a finer level: ordering only by block and using `bisect_right` included the target swap's own post-state and could include later logs from the same block. Strict block-log ordering raised the under-one-minute verdict-flip rate from 15.56% to 22.49%. A validator also has to sample the estimand's population: direct-pool swap times are opportunity snapshots, not realised multi-leg routes, however precisely their state is ordered.
...
5. **Fragmentation across venues with concentration in the asset.** Measured on clean routes: the cross-venue share of intermediated routing rises from 1.4% to 60.6% count-weighted and 15.4% to 89.4% value-weighted across the sample, while economic multi-leg incidence is broadly flat. The native-to-stable count transition from 2024 to 2026 occurs inside both single-venue and cross-venue strata, so movement into the integrated regime is not a complete composition explanation. Motivating fact and a supporting result. Caution retained: additional covered venues mechanically expand the set of cross-venue paths, and this series measures sequential paths rather than order splitting, so it cannot be called routing efficiency.
