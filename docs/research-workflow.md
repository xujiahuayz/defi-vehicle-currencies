...
5. **Fragmentation across venues with concentration in the asset.** Measured on clean routes: the cross-venue share of intermediated routing rises from 1.4% to 60.6% count-weighted and 15.4% to 89.4% value-weighted across the sample, while economic multi-leg incidence is broadly flat. Motivating fact and a supporting result. Caution retained: additional covered venues mechanically expand the set of cross-venue paths, and this series measures sequential paths rather than order splitting, so it cannot be called routing efficiency.
