...
**Validation state must be causally timed.** Reproducing a realised outcome is a valid quoter or simulator check only when every state variable contains information available immediately before that observation. Calendar-day membership is not enough. Replay all state-changing events by block and log index, and include a regression test with a state change between two observations. The V4 validator originally paired the preceding swap's price with the validation day's final liquidity map; this leaked later liquidity into earlier quotes and manufactured the only visible error in the busiest-pool sample. Interleaved replay reduced the maximum absolute error on the same 1,500 swaps to 0.000099%.
