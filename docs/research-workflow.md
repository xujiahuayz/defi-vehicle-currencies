...
- **Gas per hop must be measured from receipts.** A two-hop vehicle route is mechanically more gas-expensive than one hop, so omitting gas biases the comparison toward the vehicle route and makes direct-dominance incidence a lower bound. A single per-hop constant is still insufficient for final all-in estimates: gas must vary by route topology, venue and date, using receipts and historical gas prices. For the daily gas-price level, use dense transaction samples instead of a handful of receipts: V3 exposes hundreds of exact transaction gas prices per day through its subgraph; before V3, sample full historical blocks anchored by the V2 day calendar, falling back to the active V1 stream when an early V2 day is empty, since one block call returns gas prices for every transaction. Cache only completed day summaries and write them atomically.
