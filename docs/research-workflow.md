...
**Routing is not a site of habit, and the cross-aggregator test is already published.** Swap routing is executed by smart-order routers that are deterministic graph optimisers, so a preference for an incumbent intermediary when a cheaper direct route existed fails as evidence of trader inertia. Version 1's plan to read a wrong-signed cost coefficient as Krugman/Eichengreen inertia is retracted. Worse for the plan that replaced it: **Xi and Moallemi (arXiv 2607.20762, 22 July 2026) already ran the cross-aggregator test**, labelling routers by destination contract across Universal Router, CoWSwap, 1inch v4/v5 and Odos v2 over 2.98M WETH-USDC swaps and $120.42bn, against a three-benchmark ladder (support-constrained, full-venue, gas-aware) that arrived here as an innovation, and attributing the residual to institutional features including solver batch auctions and private order flow. We now cite it as established.
...
**What is genuinely open, and it is the actual topic.** Xi and Moallemi exclude our object by assumption, in their words: *"we restrict attention to a simplified same-token setting … we do not allow trading through other intermediate tokens when routing"*, single pair, L1 only, with multi-hop named as future work. So the intermediary-asset margin, multi-hop routing, and matched-trade head-to-head comparison are all unoccupied. Separately, **nothing in the literature connects DEX routing to the vehicle-currency question at all**: searches for vehicle currency plus DeFi routing return zero. The framing is the contribution, and the multi-hop intermediary margin is promoted from incidental to central.
...
**A condition to test before the framing is written.** The claim to overcome the FX limitation holds only if such cost-dominance windows are numerous. Measure that first. If they are, the claim is strong; if they are not, say plainly that the paper improves measurement and does not overcome the identification limitation. Settle this before the framing gets written.
...
- **Route cost: adopt Xi and Moallemi's three-benchmark ladder by name.** Suboptimality as proportional shortfall of the realised route against benchmarks solved at fixed pool state: support-constrained (reoptimise splits only across pools the trade touched, isolating mis-splitting), full-venue (reoptimise across all pools without gas, adding venue omission), and gas-aware full-venue (fixed per-pool activation cost, mixed-integer). Adopt their direction-asymmetric gas model and report medians alongside means, since a few per cent of trades drive the entire mean. Angeris, Chitra, Evans and Boyd justify the exercise: gas-aware optimal routing is mixed-integer convex, so production routers are necessarily heuristic and a shortfall exists to measure.
