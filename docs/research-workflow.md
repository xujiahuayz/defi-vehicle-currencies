...
**Source state is explicit and monotone.** A paper progresses through `temporary download -> validated PDF -> canonical literature file -> indexed text -> full-text read -> durable card -> committed and pushed`. A browser download is not saved merely because the browser lists it, and a source read from a temporary profile cannot advance the literature gate. Before reporting a source as saved or a reading as complete, validate the PDF signature, persist the exact version under `literature/papers/`, generate its page-delimited extract and index entry, and point the card at that file. The browser fetcher must consume a completed download before its context closes. Once the published article is available, remove weaker local versions and their text records unless a version comparison is itself an explicit research object.
...
The routing-efficiency attack separates four margins. Integration is the cross-venue series above. Within-vehicle search compares an exact two-leg realised route with the frontier through the same intermediary, retaining a comparison only when every frontier venue lies inside the route's observed venue set. Path selection expands that frontier to direct and alternative-vehicle paths. Vehicle rotation remains the realised intermediary-share result. The first margin is descriptive, the next two are conservative observed-venue-reach diagnostics, and none becomes a finding until the chosen pool sequence and counterfactual frontier are repriced at the same state and exact realised size. The preliminary grid matcher now carries the winning pool identities and refuses to blend quote-size endpoints when the intermediary, venue sequence or pool sequence changes. A 2022 smoke test shows why same-state is binding: the hourly matcher reports negative shortfalls on 63.2% of within-vehicle matches and 62.7% of best-path matches, which is timing contamination rather than a router beating its feasible frontier. The market-maturation test first estimates validated search and path shortfalls within fixed endpoint, vehicle, reach and notional cells, then tests whether their time compression is larger on complex or cross-venue routes, and finally adds relative search performance to the vehicle-choice specification. Aggregator attribution is conditional on executor and quote-authorship coverage; calendar improvement alone is named market routing maturation. A support-exit gate applies before every trend regression: if activity migrates out of an instrument's venue family and annual comparable-route counts collapse, the estimand loops from E back to D. Coefficients on the shrinking survivor sample remain diagnostics even when every surviving row is measured correctly.
...
3. **Rent incidence, with gas netted.** Does intermediating pay? Fee yield against LVR against net return, by pool-asset role. This is a companion distributional finding, not the transition mechanism. The clean rebuild shows that intermediation pays where both legs are major assets and loses where the native asset is paired with the long tail: native-other pools pay on 21.2% of pool-days and lose 5.82 billion dollars in aggregate. The predicted centrality curse is rejected, and its depth half fails on arithmetic because constant-product LVR as a share of capital is realised variance over eight, with depth cancelling from the rate. The size threshold reproduces, with the gas rate on repositioning pool-days falling 41-fold from the smallest capital decile to the largest, though LVR and not gas makes the long-tail pools unprofitable. The 2024-to-2026 temporal bridge rejects the proposed rotation link on admissible v2 capital: native-other capital share and profitability improve while stable vehicle routing rises. V3 virtual reserves cannot measure deposited-capital migration. See `docs/finding-rent-incidence.md`.
