# The native platform asset is a substantially better intermediary, holding the trade fixed

> **PROVISIONAL, 2026-08-06. Do not cite this estimate.** Three defects were found after it was produced and all three are being corrected. First, quote collapse differs sharply by vehicle type: a stable vehicle's quote comes back below half the notional 37.6% of the time against 9.6% for the native asset, and the gap in dominance rates, 86.7% against 58.1%, tracks that collapse gap almost one for one, so the effect may BE the collapse. Second, the estimate was produced on 4 of 7 venues, missing Curve, which is where stable-to-stable legs live and which carries roughly 85% of Uniswap v2's volume, so the collapse may be our coverage gap and not the market's thin liquidity. Curve is now validated at 0.022% median error and integrated. Third, cross-venue state was misaligned: the concentrated-liquidity venues sat at end of day while the constant-product family sat at end of hour, a median 0.345% price gap against a signal measured in tens of basis points. That is fixed. The panel is rebuilding on five venues with per-hour state, and this document will be rewritten against it.
>
> The reason to keep the file rather than delete it is that the diagnostics below are the reason the defects were found, and the earlier claims this supersedes were worse: an earlier version of the analysis called the native advantage a composition effect, and a later one read its point estimate as leaning toward the incumbent being the WORSE intermediary. Both rested on 703 identifying fixed effects out of 22,991.
