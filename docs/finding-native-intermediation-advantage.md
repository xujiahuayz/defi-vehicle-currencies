...
The realised-route panel could only compare intermediaries that *happened to be used* on the same pair the same day, which within a single venue is a coincidence: 703 cells qualified. The counterfactual panel prices the route through **every** vehicle candidate for every pair-window, so a cell contains all candidates by construction. That is a 252-fold increase in identifying cells and is the whole reason the sign resolves.

## Robustness of the control window

The cell is the control, so its width is a trade between conditioning and power and is reported rather than chosen. Window widths are integers in days, because a calendar month drifts between 28 and 31 days.

| window | cells | identifying | rows | coefficient | se | p | MDE(80%) |
|---|---|---|---|---|---|---|---|
...
| screen | rows | identifying cells | coefficient | se | p |
|---|---|---|---|---|---|
...
**Not an enumeration artefact.** Dropping the imported asset entirely, which is the most likely thin candidate, leaves the native asset beating the stable numéraire head to head by 36.8 percentage points on 9.8 million rows and 44,601 cells. Both legs of that comparison are deep and widely paired. Restricting to economically live routes, where the direct route's advantage is within 5% either way, moves the estimate only to -0.3986.
