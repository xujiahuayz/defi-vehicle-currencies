...
>
> **The objection I had not considered, which is the deepest one.** The measured gaps are not costs, because nothing prevents them being taken. A gap between a direct pool and a two-leg route in the SAME BLOCK is atomically capturable with a flash loan at zero capital, and this repository's own round-trip statistic, 12.7% of multi-leg routes by count and 21.7% by value on the median of 79 sampled days and never below 7.4% by value on any of them, establishes that the arbitrage capacity is present. Makarov and Schoar devote a full section of a JFE paper to explaining why their 15-to-40% Korean premium survived, naming settlement latency, short-sale unavailability and capital controls, and none of those frictions exists between two routes in one block. A median same-state gap of -2,459 basis points is therefore more likely off-support quoter error than economics, and the quoter is validated on swaps that happened and applied to 123.8 million that did not. Adding Curve does not address this.
>
> **Why the estimand itself is wrong.** "Native intermediation is cheaper conditional on the trade" is close to restating the definition of the native platform asset as the asset with the thickest pairing network, which is the maintained assumption of the literature this paper cites, so a confirmation is not a contribution. What the project has actually established and what FX data can never contain is the STATE in which an asset holds the vehicle role while being strictly cost-dominated, common here at 17.9% of routes gross and 30.0% all-in. The publishable question is how long the role SURVIVES once it stops being the cheapest route, priced in dollars foregone and days of delay, with the return-edge against displacement-edge asymmetry separating hysteresis from persistence. That has a time dimension, economic units and an asymmetry test, and it cannot be restated as a definition.
>
