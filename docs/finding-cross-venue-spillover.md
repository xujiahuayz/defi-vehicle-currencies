...
Node C established on 2026-08-06 that the LEVEL of betweenness centrality is close to a restatement of this project's definition of the native asset, since `asset_types.py` defines native as the asset with the thickest incumbent pairing network, which is degree, and the betweenness leader equals the degree leader on 15 of 15 sampled days. That constraint binds here and is stated where the numbers are used. The pre-event base of 0.689 for the native-minus-stable betweenness share is therefore partly definitional and carries no independent information. What is used in this document is the CHANGE in that share around a date, and a definition that fixes which asset leads does not fix the time path of the gap or say anything about whether an architecture change on another venue moves it. The same node records that the justification for betweenness by appeal to Flandreau and Jobst rests on a summary, since that paper is not in the corpus, so no citation is claimed for the measure here.
