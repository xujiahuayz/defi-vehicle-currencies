...
All numerical estimates below describe the superseded pre-repair generation and are retained only as an audit trail. None is admitted to the findings freeze. The V2 return path is now fail-closed because its variance came from each pool's own marginal price, while the cited LVR estimand requires an independent external reference-price path. Exact prior-calendar capital and provider-reserve reconciliation remain necessary but do not repair that mismatch. V3 provider TVL is no longer an admitted capital measure: a historical on-chain balance probe produced an actual-balance/reported-TVL ratio with median 0.523, and only 3 of 14 critical pool-days matched within 25%. Every V2 and V3 fee-to-LVR, net-flow and return statistic below is retired. V2 re-enters only after external-reference variance passes; V3 additionally requires event-complete inventory replay, historical custody checks, removal of protocol-owned claims and path-integrated concentrated-liquidity LVR.
...
For V2, fee revenue is 30 basis points times USD volume. The superseded implementation applied the constant-product closed form of Milionis, Moallemi, Roughgarden and Zhang to variance from the pool's own hourly marginal price. That is not the source's LVR estimand: the required variance path is an independent external-market price. The old pool-price object remains only a visible sensitivity diagnostic and supplies no LVR, net-return or profitability statistic. A future admitted LVR numerator is external-reference realised variance over eight times contemporaneous pool value. Gas is the observed count of mints and burns times a per-operation gas figure times the day's median gas price times the ETH price, charged at pool level against pool-level fee revenue. The return denominator remains separately sourced exact-lag deposited capital, so an admitted LVR return would be $(W_t/C_{t-1})\mathrm{RV}^{\mathrm{ext}}_t/8$; neither $W_t$ nor local depth silently replaces $C_{t-1}$.
