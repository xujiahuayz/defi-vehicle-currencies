...
Exchange-to-token resolution rate by direct lookup: **0%**. Not "low" — zero, and not by accident of coverage. The raw fetch in `src/ddvc/fetch/schemas.py` requested `exchangeAddress` and never `tokenAddress`; the V1 `meta` files hold only day, block range, subgraph id, and row counts; `uniswap_v1` is absent from `data/unified/`, whose eight sources are uniswap v2 to v4, sushiswap v2 and v3, curve, balancer and fluid; and a grep for V1 exchange addresses across this repo and the two sibling reference repos returns nothing. V1 exchange addresses are not derivable either, since the factory used `CREATE`, whose output depends on a nonce. Section 4 covers the statistical substitute and why it fails.
