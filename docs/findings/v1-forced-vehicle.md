...
**Even a clean horizon profile would not measure inertia.** Routing is executed by graph optimisers, so preferring ETH when a direct pool exists is not evidence of habit. A newly created direct pool can be thin, and routing through the deepest network path can remain cost-optimal. The separately registered transaction-state counterfactual is complete on 73 monthly dates across exact Uniswap V2, SushiSwap V2, and Uniswap V3 states. At least 120 days after entry, the entry vehicle is retained on 93.4% of routes when it remains the price leader and 22.4% across 8,807 routes in 1,311 pairs when the other family leads. Pair-day weighting gives 94.6% and 24.5%. Current public prices therefore explain a large part of vehicle retention. The remaining comparison is gross of gas and excludes private flow and omitted venues, so the residual share does not identify inertia.
