...
**Even a clean horizon profile would not measure inertia.** Routing is executed by graph optimisers, so preferring ETH when a direct pool exists is not evidence of habit. A newly created direct pool can be thin, and routing through the deepest network path can remain cost-optimal. The separately registered transaction-state counterfactual is now complete on 73 monthly dates across exact Uniswap V2, SushiSwap V2, and Uniswap V3 states. At least 120 days after entry, the entry vehicle is retained on 99.5% of routes when it remains the price leader and only 12.0% across 15,308 routes in 1,886 pairs when a challenger leads. This closes the cheaper-route caveat in the direction that matters: current public prices explain vehicle retention sharply, rather than revealing routes that commonly ignore a cheaper challenger. The comparison remains gross of gas and excludes private flow and omitted venues, so it still identifies no inertia effect.
...
The former missing inputs are now complete. The V1 exchange registry supplies exact token identities; the transaction-time quoter is joined to the pair-entry panel; and the paper's principal formation analysis runs on the unified nine-deployment route layer rather than the V2-only persistence panel. The V2 result remains institutional evidence about the venue that removed the mandate, not the paper's identification spine. The remaining boundary is narrower and explicit: exact prices cover three venues on monthly dates, while private flow, gas, and pool families without reconstructible historical state remain outside the counterfactual opportunity set.
