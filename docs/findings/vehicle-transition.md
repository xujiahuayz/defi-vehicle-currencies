...
The matched-ultimate-pair decomposition pools ultimate pairs with and without a feasible stable bridge, so its small within-pair component is not an availability-conditioned substitution estimate. The bridge-establishment design instead follows 865 ordered ultimate-pair and route-scope events that previously routed through WETH, had no earlier observed stablecoin route, and first acquired persistent DAI, USDC, or USDT support on both atomic pairs in Uniswap V2 or SushiSwap V2. Relative to the prior 30 days, stablecoin route-count share rises by 7.93 percentage points in the first month (two-way-clustered SE 1.88 points) and supported-value share rises by 3.11 points (SE 1.53 points). WETH nevertheless retains 92.6% of route counts and 97.7% of supported value. Stable-bridge feasibility therefore produces a statistically detectable but partial redistribution of routed activity.

The route-count shift persists over days 30--119: the paired increase is 8.68 percentage points (SE 2.57 points). The supported-value increase over that horizon is 6.94 points (SE 4.53 points), which is less precise. Native-route intensity falls in both windows, while total route-count intensity also contracts, so the estimates do not imply one-for-one diversion from WETH. The event is defined by observed prior-calendar deposited-capital support, rather than an exogenous pool-creation shock, and remains descriptive. Separate USDC and USDT interactions rank already-feasible stablecoin alternatives relative to DAI; they do not estimate the effect of bridge establishment.
