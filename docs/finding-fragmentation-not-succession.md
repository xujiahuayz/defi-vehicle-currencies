...
>
> **A citation error, now resolved, and it cuts against the statistic it was cited to defend.** The betweenness definition was justified in two committed files by appeal to Flandreau and Jobst (2009) modelling currency use as a network with externalities. That paper was NOT IN THE CORPUS when the sentence was written, so it was cited from memory of a summary. The author-deposited precursor has since been obtained and read, and it does not support betweenness: `grep -aoic` over the extract returns 0 for centrality, 0 for betweenness, 0 for eigenvector and 0 for shortest path, the paper's own term is "strategic externalities" at 14 occurrences against 0 for "network externalities", and its measure of a currency's international role is the number of foreign markets quoting it. That is DEGREE. So the one prior paper measuring the international currency role on a network measures it with the same statistic `src/ddvc/asset_types.py` uses to DEFINE the native asset, which deepens the circularity above instead of answering it.
>
