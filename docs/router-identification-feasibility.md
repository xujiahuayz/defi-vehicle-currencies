...
**Rebuilt-panel figures, 2026-08-16.** The numbers in the previous paragraph predate the corrected route rebuild and are retained as the pre-rebuild record. On the current certified `output/exhibits/routing_technology_windows.jsonl`, the same windows give a cross-venue share, taken over intermediated routes, of 8.52% to 10.97% at Auto Router v1, 11.33% to 14.39% at the cross-version release and 26.38% to 24.40% at Universal Router; indirect-route incidence over economic routes moves 22.65% to 18.95%, 18.47% to 18.63% and 19.77% to 19.49%. Every sign is unchanged, so the paragraph's conclusion survives the rebuild. Two facts the pre-rebuild record did not state are now used in the manuscript. True intermediation, as distinct from indirect routing, moves -5.67, +0.83 and -0.71 percentage points, so no release is followed by a materially higher incidence of intermediation. Mean legs per indirect route move by at most 0.049 across the six windows. The balanced five-venue perimeter reproduces the full perimeter exactly in every window because the later entrant venues have no route component before 2023-04-05; a window moved past that date would no longer hold the venue set fixed, and `ddvc.venue_tables.routing_window_values` refuses the table and macros if the two perimeters ever diverge. Section 5 of the manuscript consumes these figures as descriptive window composition and states the design limits, including the thirty-day overlap between the first release's later window and the second's earlier window, and the November 2022 centralised-exchange failure inside the third release's earlier window.
