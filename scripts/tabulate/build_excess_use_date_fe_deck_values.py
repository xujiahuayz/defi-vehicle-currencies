...
CODE_SOURCES = ["scripts/tabulate/build_excess_use_date_fe_deck_values.py"]
...
def render(ladder: pd.DataFrame, screens: pd.DataFrame) -> str:
    lines = [
        "% Generated by scripts/tabulate/build_excess_use_date_fe_deck_values.py.",
        ...
        "% fixed effects absorbing the calendar. Supporting scope: the estimates run",
        ...
        "% scripts/analyze/run_excess_use_date_fe_ladder.py then this builder.",
    ]
    for stem, spec, sample, term in ESTIMATES:
        ...
        lines += [
            ...
            f"\\newcommand{{\\DateFE{stem}P}}{{{_pvalue(p_value)}}}",
        ]
        if stem in TABLE_STEMS:
            scale = 1.0
            lines += [
                f"\\newcommand{{\\DateFE{stem}Table}}{{${scale * float(row['beta']):+.2f}{_stars(p_value)}$}}",
                f"\\newcommand{{\\DateFE{stem}TableSE}}{{{scale * float(row['se']):.2f}}}",
            ]
