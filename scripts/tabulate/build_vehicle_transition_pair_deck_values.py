...
CODE_SOURCES = ["scripts/tabulate/build_vehicle_transition_pair_deck_values.py"]
...
# of the same validated ledger, never a re-estimation.
...
def _market_incidence_classes(
    ...
    estimated: every quantity is a cell of the validated support ledger, and the
...
def _margin_breadth(
    ...
    losses of similar size cancelled. Both are computed from the same validated
...
def _cohort_endpoint_margins(
    ...
    the same validated rows the cohort reading already used, and the function
