...
def render_liquidity_provision_behavior_deck_values(estimates: pd.DataFrame) -> str:
    ...
    usdc_v3_lp_total_month = _single(
        estimates,
        record_type="route_capital_gap_v3_lp_action_candidate_specific",
        horizon_days=30,
        outcome="future_log1p_v3_total_lp_actions",
    ...
    usdt_v3_lp_total_month = _single(
        estimates,
        record_type="route_capital_gap_v3_lp_action_candidate_specific",
        horizon_days=30,
        outcome="future_log1p_v3_total_lp_actions",
    ...
    stable_v4_lp_actions = _single(
        estimates,
        record_type="lp_action_protocol_comparison_group",
        candidate_group="stable_candidates",
    )
    all_v4_lp_actions = _single(
        estimates,
        record_type="lp_action_protocol_comparison_group",
        candidate_group="all_candidates",
    )
    usdt_v4_lp_actions = _single(
        estimates,
    ...
    if not (
        ...
        and float(usdt_v3_lp_total_month["coefficient"]) > 0
        and float(dai_v3_lp_total_month["p_value"]) < 0.01
        and float(usdc_v3_lp_total_month["p_value"]) < 0.01
    ...
    if not (
        float(all_v4_lp_actions["v4_action_assignments"]) > 100_000
        and float(stable_v4_lp_actions["v4_action_share"])
        > float(stable_v4_lp_actions["v3_action_share"])
        and float(usdt_v4_lp_actions["v4_action_share"])
        > float(usdt_v4_lp_actions["v3_action_share"])
        and float(all_v4_lp_actions["v4_narrow_medium_share"]) > 0.50
    ...
    lines = [
        ...
        f"\\newcommand{{\\LiqBehVFourActionAssignments}}{{{_integer(int(all_v4_lp_actions['v4_action_assignments']))}}}",
        f"\\newcommand{{\\LiqBehVFourStableActionShare}}{{{_pct(float(stable_v4_lp_actions['v4_action_share']))}}}",
        f"\\newcommand{{\\LiqBehVThreeStableActionShare}}{{{_pct(float(stable_v4_lp_actions['v3_action_share']))}}}",
        f"\\newcommand{{\\LiqBehVFourUsdtActionShare}}{{{_pct(float(usdt_v4_lp_actions['v4_action_share']))}}}",
