#!/usr/bin/env python3
"""Render the V3-versus-V4 LP-flow protocol-contrast table."""
...
def render_v3_v4_lp_flow_protocol_contrast(results: pd.DataFrame) -> str:
    required = {
        "horizon_days",
        "outcome",
        "term",
    ...
    if missing:
        raise ValueError(f"V3/V4 LP-flow protocol-contrast results lack columns: {missing}")
    contrasts = results[
    ...
    if contrasts.empty:
        raise ValueError("V3/V4 LP-flow protocol-contrast table has no contrast rows")
    duplicates = contrasts.duplicated(["horizon_days", "outcome"])
    if duplicates.any():
        raise ValueError("V3/V4 LP-flow protocol-contrast table rows are not unique")
    ...
    if observed != expected:
        raise ValueError(
            "V3/V4 LP-flow protocol-contrast table row set is incomplete: "
            f"missing={sorted(expected - observed)}"
        )
    if set(contrasts["fixed_effects"]) != {"candidate_date+protocol"}:
        raise ValueError("unexpected fixed effects in V3/V4 LP-flow protocol contrast")
    if contrasts["activity_controls"].nunique() != 1:
        raise ValueError("V3/V4 LP-flow protocol-contrast table mixes controls")
    ...
    if len(observations) != 1 or len(clusters) != 1:
        raise ValueError("V3/V4 LP-flow protocol-contrast table mixes support counts")
    rows.append(
    ...
    rows.append(r"Asset-date and protocol effects & \multicolumn{3}{r}{Yes} \\")
    rows.append(r"Current protocol LP-flow controls & \multicolumn{3}{r}{Yes} \\")
...
def main() -> int:
    ...
    write_table_artifacts(
        "v3_v4_lp_flow_protocol_contrast",
        render_v3_v4_lp_flow_protocol_contrast(results),
        preview_width="7.8in",
    )
    return 0
