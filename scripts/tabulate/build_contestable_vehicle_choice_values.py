...
def render_contestable_vehicle_choice_values(
    ...
    same_sample_output_advantage = _single(
        estimates,
        record_type="contestable_vehicle_choice_regression",
    ...
    if not (
        ...
        and float(same_sample_output_advantage["coefficient_pp"]) > 0
    ...
    if int(same_sample_output_advantage["observations"]) != int(
        output_advantage["observations"]
    ):
        raise ValueError("price-only and price-capital models use different samples")
    ...
    lines = [
        ...
        f"\\newcommand{{\\ContestSameSampleOutputAdvantageHundredBpEffect}}{{{_signed_pp(float(same_sample_output_advantage['coefficient_pp']))}}}",
        f"\\newcommand{{\\ContestSameSampleOutputAdvantageHundredBpSE}}{{{_unsigned_pp(float(same_sample_output_advantage['standard_error_pp']))}}}",
        f"\\newcommand{{\\ContestSameSampleOutputAdvantageHundredBpN}}{{{_integer(same_sample_output_advantage['observations'])}}}",
