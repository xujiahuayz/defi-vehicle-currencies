...
| `render_within_day_ladder.py` | Certified `excess_use_date_fe_ladder.jsonl` and sidecar | Current paper body for `tab:within-day-ladder`; selects each cell by specification, sample and term rather than row order, so a five-token magnitude cannot be printed in a thirty-seven-token cell. |
...
| `tab:within-day-ladder` | `paper/sections/03-dominance.tex` inputs `output/tables/within_day_ladder.tex` | Certified `excess_use_date_fe_ladder.jsonl` | Active generated body with a one-to-one renderer and provenance-stamped TeX/PDF outputs. Provisional E0 scope: the ladder runs on the green route-only D3 panel and is not submission-authoritative until the findings freeze admits it. |
