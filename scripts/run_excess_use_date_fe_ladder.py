...
Mechanicalness screens run in the same file and are reported beside the estimates,
because intermediary share and endpoint share are drawn from the same day's route
universe. `scripts/run_dominance_mechanicalness_screen.py` is the screen owner for
the route-cost dominance design and cannot carry this one: it reads
`route_cost_panel_v2` and estimates through `pyfixest`, which this environment does
not install. The three screens here are the same idea instantiated for the
share-on-share specification and are documented at `screens()`.
...
CODE_SOURCES = [
    "scripts/run_excess_use_date_fe_ladder.py",
