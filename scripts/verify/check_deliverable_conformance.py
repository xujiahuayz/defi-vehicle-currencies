...
done. `README.md` names it as the gate that closes the writing loop.

  ./scripts/run scripts/verify/check_deliverable_conformance.py            check
  ./scripts/run scripts/verify/check_deliverable_conformance.py --brief    also print an agent brief
...
# not a substitute for editorial judgment. They can show that the draft uses a known construction
...
# weakened without an explicit withdrawal recorded in README.md.
...
STAGES = [
    ("venue-structure alarm against the exemplars",
     [str(PY), "scripts/verify/measure_venue_optics.py"],
    ...
    ("known-construction alarm against the exemplars",
     [str(PY), "scripts/verify/measure_prose_conventions.py"],
    ...
    ("discovered over-used constructions, nobody naming them first",
     [str(PY), "scripts/verify/find_prose_outliers.py"],
    ...
    ("prose shape against the venue's own distributions",
     [str(PY), "scripts/verify/measure_venue_shape.py"],
    ...
    ("generated evidence in the deck",
     [str(PY), "scripts/verify/audit_deck_evidence.py"],
     "a scientific plot or measured value is typed into slide source instead of generated through output/", True),
    ("source path for every empirical number",
     [str(PY), "-m", "pytest", "tests/test_paper_evidence.py", "-q"],
     "a measured number names no generated output, or names one that does not exist", True),
