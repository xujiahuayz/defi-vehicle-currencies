...
from ddvc.datasets import validate_before_install, state_partitions  # noqa: E402
...
CODE_SOURCES = [
    "scripts/verify/validate_weighted_quoter.py",
...
def main() -> int:
    ...
    state_release = state_partitions("multi_asset", "balancer", MULTI_ASSET_COLUMNS)
    ...
    write_exhibit(
        ...
        inputs=list(state_release.input_paths),
        notes=f"held-out Balancer quote errors from canonical transaction-state partitions; released-state identity {state_release.label}",
        preinstall_validator=validate_before_install(state_release),
    )
