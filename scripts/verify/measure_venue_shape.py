...
it does not by itself establish a defect.
  ...
  ./scripts/run scripts/verify/measure_venue_shape.py                    corpus diagnostics and draft
  ./scripts/run scripts/verify/measure_venue_shape.py --section 03       one-section diagnostic
...
def main() -> int:
    ...
    if out_of_band:
        ...
        print("  scripts/verify/measure_prose_conventions.py   constructions to avoid")
        print("  scripts/verify/find_prose_outliers.py         tells nobody named in advance")
    else:
