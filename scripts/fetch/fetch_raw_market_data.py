...
Examples:
  ...
  ./scripts/run scripts/fetch/fetch_raw_market_data.py audit-genesis --dex all
  ./scripts/run scripts/fetch/fetch_raw_market_data.py fetch --dex uniswap_v3 --start genesis --end <exclusive-end>
  GRAPH_API_KEYS=... ./scripts/run scripts/fetch/fetch_raw_market_data.py fetch --dex all --streams swaps daily mints burns modify_liquidities hourly_reserves
...
def enrich_v4_statics_day(day: dt.date) -> dict[str, object]:
    """Fill quote statics without replacing signed amounts or block evidence."""
...
def coverage_report(names: list[str], end_by_source: dict[str, dt.date]) -> dict[str, dict[str, object]]:
    ...
    for name in names:
        ...
        for day in days:
            ...
            indexed: set[str] = set()
            if metadata.exists():
                indexed = indexed_metadata_streams(
                    metadata, expected_paths=expected_paths
                )
            for stream in streams:
...
def coverage_has_gaps(report: dict[str, dict[str, object]]) -> bool:
    """Whether any consumer-required source lacks a file or row ledger."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.choices["fetch"].add_argument("--missing-files-only", action="store_true", help="With --gaps-only, fetch only absent payload files and leave existing payloads untouched.")
    ...
    sub.choices["coverage"].add_argument(
        "--strict",
        action="store_true",
        help="exit nonzero if any required payload or metadata row ledger is missing",
    )
