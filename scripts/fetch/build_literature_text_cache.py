...
Writes  literature/text/<stem>.txt          one file per paper, pages delimited
        literature/text/_index.jsonl        stem, title, pages, characters, PDF bytes
"""
...
def validate_corpus(
    ...
    for stem in sorted(expected & set(pdfs)):
        recorded = records[stem].get("pdf_bytes")
        if not isinstance(recorded, int) or recorded < 1:
            errors.append(f"missing PDF byte count: {stem}")
        elif pdfs[stem].stat().st_size != recorded:
            errors.append(f"PDF byte count changed: {stem}")
    return errors
...
def main() -> int:
    ...
    ap.add_argument("--check-corpus", action="store_true",
                    help="verify local PDF/text names and PDF byte counts against the tracked index")
    args = ap.parse_args()
