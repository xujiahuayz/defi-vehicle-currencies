...
from ddvc.datasets import expected_route_days, require_datasets
from ddvc.panel_assembly import assemble_parquet_shards
from ddvc.paths import DATA_DIR, REPO_ROOT, TOKEN_PRICE_DAILY_PANEL, TOKEN_PRICE_LOCK
...
CODE_SOURCES = [
    "scripts/process/build_token_price_panel.py",
...
def _write_day(day: str, root: Path, force: bool) -> tuple[str, int]:
    ...
    source = UNIFIED / f"{day}.parquet"
    code_mtime = max((REPO_ROOT / path).stat().st_mtime_ns for path in CODE_SOURCES)
    current_after = max(source.stat().st_mtime_ns, code_mtime)
    if output.exists() and not force and output.stat().st_mtime_ns >= current_after:
        return day, pq.ParquetFile(output).metadata.num_rows
    legs = pq.read_table(source, columns=PRICE_COLUMNS).to_pandas()
