...
def fetch_missing_chunks(
    ...
    *,
    ...
    pending = [
        item
        for item in ranges
        if force or not exact_chunk_complete(venue, day, *item, frozen_upper=frozen_upper)
    ]
...
def reconcile_day(
    ...
    *,
    ...
    if start_block is None:
        ...
        if outside:
            raise ValueError(f"provider events fall outside the requested block bounds: {venue}/{day}")
