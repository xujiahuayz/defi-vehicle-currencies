...
from ddvc.provenance import stamp, verify
...
PROVISIONAL_RESULTS = OUTPUT_DIR / "provisional" / "route_methodology_robustness.jsonl"
PROVISIONAL_RESAMPLING = (
    OUTPUT_DIR / "provisional" / "route_methodology_distribution_resampling.jsonl"
)
PROVISIONAL_HETEROGENEITY = (
    OUTPUT_DIR / "provisional" / "route_methodology_heterogeneity.jsonl"
)
PROVISIONAL_HETEROGENEITY_DECK_VALUES = (
    OUTPUT_DIR / "provisional" / "route_methodology_heterogeneity_deck_values.tex"
)
DEFAULT_D3_CERTIFICATE = (
    REPO_ROOT
    / "data/processed/d3_analysis_release/generations/"
    / "25c755ae635d90846fb754e5cb58caa7f377787daba0743d95667b8c29ed9e6c"
    / "certificate.json"
)
CODE_SOURCES = [
...
def provisional_snapshot_identity(
    panel_path: Path, certificate_path: Path
) -> dict[str, str]:
    """Admit an unchanged derived panel when only its released-source lease drifted.

    This deliberately does not reopen the source release.  It supports E0 diagnostics
    from the immutable panel bytes while a cross-host transfer or release republish is
    in progress.  Any panel-code drift, panel-content drift, or changed input outside
    the certificate's exact release-member perimeter blocks estimation.
    """

    certificate = json.loads(certificate_path.read_text(encoding="utf-8"))
    panel_verdict = verify(panel_path)
    allowed_drift = {
        path
        for record in certificate.get("claim_inputs", [])
        if record.get("input_kind") == "release_pointer"
        for member in record.get("release_artifacts", [])
        for path in (str(member.get("path") or ""),)
    }
    changed_inputs = set(panel_verdict.get("changed_inputs") or [])
    if (
        certificate.get("status") != "pass"
        or certificate.get("specification_stage") != "design_seed"
        or "vehicle_transition" not in certificate.get("executable_claim_ids", [])
        or not panel_verdict.get("content_current")
        or not panel_verdict.get("code_current")
        or not changed_inputs.issubset(allowed_drift)
    ):
        raise RuntimeError(
            "provisional route-method snapshot has semantic, code, content, or "
            "out-of-perimeter input drift"
        )
    endpoint_generations = {
        str(record.get("release_generation") or "")
        for record in certificate.get("claim_inputs", [])
        if record.get("input_kind") == "release_pointer"
    }
    generation = str(certificate.get("generation") or "")
    if len(endpoint_generations) != 1 or not generation:
        raise RuntimeError("provisional D3 certificate has no unique generation identity")
    endpoint_generation = endpoint_generations.pop()
    if not endpoint_generation:
        raise RuntimeError("provisional D3 certificate has an empty endpoint generation")
    return {
        "d3_generation": generation,
...
def run(
    *,
    ...
    root: Path = REPO_ROOT,
    environment=None,
    provisional_certificate: Path | None = None,
    ...
    notes = (
        ...
        "robustness on matched realised-route support; no opportunity-set or causal "
        "interpretation"
    )
    if provisional_certificate is None:
        ...
        identity = provisional_snapshot_identity(panel_path, provisional_certificate)
        generation = identity["d3_generation"]
        endpoint_generation = identity["endpoint_generation"]
        provisional_note = (
            f"PROVISIONAL E0 ONLY; d3_generation={generation}; "
            f"endpoint_generation={endpoint_generation}; {notes}; clean J0 rerun required"
        )
        inputs = [panel_path, provisional_certificate]
        write_exhibit(
            results,
            result_path,
            code_sources=CODE_SOURCES,
        ...
        write_exhibit(
            ...
            inputs=inputs,
            notes=(
                "PROVISIONAL E0 ONLY; two-way ordered-pair and month-day cluster "
                f"sign-randomisation draws; d3_generation={generation}; clean J0 rerun required"
            ),
        )
...
def render_heterogeneity_deck_values(
    ...
    *,
    ...
    """Render source-labelled provisional macros for the WETH eligibility frame."""
    ...
    lines = [
        "% Generated by scripts/run_route_methodology_robustness.py; do not edit.",
        "% EVIDENCE-STATUS: E0 pending clean J0; provisional mechanical-eligibility packet",
        f"% EVIDENCE-D3-GENERATION: {d3_generation}",
        f"% EVIDENCE-ENDPOINT-GENERATION: {endpoint_generation}",
...
def run_heterogeneity(
    *,
    panel_path: Path = PAIR_PANEL,
    result_path: Path = PROVISIONAL_HETEROGENEITY,
    deck_values_path: Path | None = PROVISIONAL_HETEROGENEITY_DECK_VALUES,
    provisional_certificate: Path = DEFAULT_D3_CERTIFICATE,
) -> pd.DataFrame:
    """Write the isolated E0 eligibility and heterogeneity packet."""
    ...
    write_exhibit(
        ...
        inputs=[panel_path, provisional_certificate],
        notes=(
            "PROVISIONAL E0 ONLY; predeclared realised-integration-scope heterogeneity, "
            "appendix-only mechanically bounded baseline-state diagnostics, the deterministic "
            "WETH-endpoint eligibility identity, and full-versus-exclude-WETH comparisons "
            "with two-way clustered or calendar-HAC "
            f"inference and Holm control; d3_generation={identity['d3_generation']}; "
            f"endpoint_generation={identity['endpoint_generation']}; the joint USDT-USDC-WETH "
    ...
    if deck_values_path is not None:
        ...
        stamp(
            deck_values_path,
            code_sources=CODE_SOURCES,
            inputs=[result_path, panel_path, provisional_certificate],
            rows=len(results),
            notes=(
                "PROVISIONAL E0 deck macros for the mechanical WETH-endpoint allocation "
                "comparison; generation identities remain source comments; clean J0 rerun required"
            ),
        )
    return results
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="write an immutable-snapshot E0 diagnostic under output/provisional",
    )
    parser.add_argument(
        "--provisional-certificate", type=Path, default=DEFAULT_D3_CERTIFICATE
    )
    args = parser.parse_args()
    result_path = PROVISIONAL_RESULTS if args.provisional_snapshot else args.results
    resampling_path = (
        PROVISIONAL_RESAMPLING if args.provisional_snapshot else args.resampling
    )
    results, _draws = run(
        ...
        provisional_certificate=(
            args.provisional_certificate if args.provisional_snapshot else None
        ),
    )
    if args.provisional_snapshot:
        run_heterogeneity(
            panel_path=args.panel,
            result_path=PROVISIONAL_HETEROGENEITY,
            provisional_certificate=args.provisional_certificate,
        )
