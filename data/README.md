...
Every newly stamped derived artifact binds its complete payload digest. Parquet panels additionally bind their physical row count, ordered columns and serialized Arrow schema; JSON Lines artifacts bind valid nonblank JSON records, including a final record without a trailing newline. The declared row count must equal the physical payload before publication and again when read. `ddvc.journaled_publication` owns prepared-to-committed installation of fully staged filesystem bundles; the three registered counterfactual bundles remain on `ddvc.counterfactual_publication` until they are migrated. Rollback state survives a process crash, and cleanup occurs only after the old or new bundle is verified. Writers own the payload and sidecar as one exclusive perimeter, and consumers hold one shared lease over every declared payload and sidecar through the complete read. A mixed pair after a crash or manual edit fails closed. Legacy artifacts without a complete digest are not current and require a controlled rebuild or exact-payload restamp; never update only their code, input or row metadata.
