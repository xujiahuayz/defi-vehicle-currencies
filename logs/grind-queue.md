...
- [ ] **JAVA INTERJECTION (WeCom via glotl, 2026-08-16T15:50Z):** BUILD THE NON-CHRONOLOGICAL REGRESSION SUITE — this is estimators, not deck framing. Java's verbatim objection, raised for the third time: "Do you have a suite of regressions to run? Like I said too much chronologically based comparisons still!" The 2026-08-16 deck pass answered the *presentation* complaint (time-axis frames 2 → 1) and explicitly ran no new estimator. The complaint is now about what is estimated.

  **Diagnosis the supervisor verified in the working tree, 2026-08-16.** The reason the science still reads chronologically is structural, not stylistic. Commit `68d4df7` ("gate findings on validated panel diagnostics", 2026-08-06) withheld most route-cost-panel estimators on definitional grounds, and the withheld set is disproportionately the *cross-sectional* one. `scripts/refresh_panel_dependents.py` now owns exactly three stages (`measure_quoter_support.py`, `measure_dominance_windows.py`, `test_gap_arbitrage_bound.py`); `audit_findings_freeze.py` retires `run_vehicle_dominance_hdfe.py` (retired vehicle definition), `run_survival_after_dominance.py` and `run_displacement_asymmetry.py` (the two duration arms compare asymmetric events), `measure_realised_dominance.py` (route-level realised incidence is not the required pair-candidate-period unit; the realised join omitted quote hour), `run_dominance_specification_curve.py` (mixes estimands), `run_jfe_construct_validity_checks.py` and `build_paper_exhibits.py`. Separately, `docs/findings-freeze.md` records the route-cost panel itself as **withdrawn pending the node-D rebuild**. Net effect: the only fully certified layer is route-only D3, the calendar contrast is what route-only D3 supports, so the paper drifted onto the time axis by default.

  **Do not** resurrect any retired estimator by re-running it, and **do not** wait on the route-cost panel for the items below. Everything in (1)–(3) runs on green route-only D3 today.

  **(1) Promote excess use to a lead result, because it is cross-sectional by construction.** Vehicle excess use (intermediary share normalised by endpoint-demand share) asks whether an asset intermediates beyond what its own trade demand implies, at a point in time, with no calendar axis at all. The freeze registry already carries 2026 values of 1.41 count / 1.19 strict value for stable against 0.77 / 0.68 for native. Read the current figures from the exhibit, never from this queue text (the 2026-08-16 pass caught queue count figures not reproducing). State it as a single-cross-section claim with its uncertainty, and give it a frame and a paper passage at the same altitude as the rotation.

  **(2) Build the backing-regime and token cross-sections as their own arm.** The registry already separates fiat-reserve stables (96.1% of supported stable intermediary value, excess use 1.06), on-chain-collateralised (0.06) and synthetic (0.49), and separates USDC (already a vehicle, 1.50 → 1.53 count; 1.12 → 1.14 strict value) from USDT (1.06 → 1.23; 0.59 → 1.42). Cut this on backing mechanism and on venue-integration scope, not on year. The economic question is *which kind of asset* takes the role, and that is a contrast across candidates within a period.

  **(3) Convert the endpoint-to-endpoint contrast into a repeated cross-section.** Wherever a result is currently "2024 versus 2026", also estimate it date by date across the 2,332-day release and report the distribution of date-specific coefficients with its dispersion, rather than two endpoints joined by an arrow. This keeps the fact and removes the two-point comparison as the identifying variation. Report how many headline numbers change materially under this cut.

  **(4) Rescue the retired non-calendar designs by re-specifying them, in this priority order.** These are the arms that would answer Java properly, and each was retired for a *definitional* defect, not a design defect, so each is repairable without new acquisition. (a) The dominance horse-race holding the trade fixed: re-specify at the required pair-candidate-period unit with an admitted vehicle definition, restoring the within pair-by-window-by-size comparison across candidates. (b) The two duration arms: the retirement reason is that retention and displacement compare asymmetric events, which is the hysteresis claim's whole content, so define the symmetric event pair explicitly before re-estimating, or state in the paper that asymmetry is not identifiable on current inputs. Land each behind a locked specification in `docs/findings-freeze.md` before it re-enters `refresh_panel_dependents.py`.

  **(5) State the blocked set plainly rather than quietly substituting time for it.** Fixed-opportunity route cost, hysteresis, V4 gross-to-net settlement and the LP-side arms are blocked on their purpose-bound J0 inputs. Say so in the paper's design section as a scope limit. Do not let a calendar comparison stand in for a design that is merely waiting on data.

  Guardrails: single draft refreshed in place, no fork; every number binds through generated macros from its exhibit and never from this queue text; the "not X, but Y" rule from 2026-08-16 applies to every contrast produced here (both sides evidenced in the same frame and units, negated side carried by an interval that excludes the economically relevant magnitude, otherwise dropped for a plain descriptive line). Report in the ledger the before/after count of headline results whose identifying variation is calendar time.
