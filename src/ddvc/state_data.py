...
STATE_ROOT = DATA_DIR / "processed" / "market_state"
...
def _state_partition_inputs(
    ...
    provider_inputs = [
        raw_stream_path(raw_root, venue, stream, day)
        for stream, _kind, _sign in FAMILY_STREAMS[family][venue]
        if include_absent or raw_stream_path(raw_root, venue, stream, day).exists()
    ]
...
def iter_normalised_cp_reserve_records(
    ...
    if not path.is_file():
        raise FileNotFoundError(f"validated reserve stream is missing: {venue}/{day}")
        ...
        with gzip.open(path, "rt") as handle:
            for line in handle:
                ...
                if records and records[0]["block_number"] is not None and records[0]["log_index"] is not None:
                    ...
                    event_values = json.dumps(records, sort_keys=True, default=str)
                    prior = event_orders.get(key)
                    if prior is not None:
                        if prior == event_values:
                            counters["duplicate_events"] += 1
                        else:
                            counters["conflicting_events"] += 1
                        continue
                    event_orders[key] = event_values
                rows.extend(records)
...
def bind_state_partition_output(
    ...
    """Record the installed panel size for a cheap completeness check."""

    stat = panel_path.stat()
    return replace(quality, output_bytes=stat.st_size)


def state_partition_output_is_current(
    quality: StatePartitionQuality, panel: Path
) -> bool:
    return panel.is_file() and quality.output_bytes == panel.stat().st_size


def _inputs_not_newer(inputs: list[Path], panel: Path) -> bool:
    """A direct panel is stale when a declared source is absent or newer."""

    if not panel.is_file():
        return False
    built_at = panel.stat().st_mtime_ns
    return all(path.is_file() and path.stat().st_mtime_ns <= built_at for path in inputs)
...
def _read_state_quality(
    *,
    ...
    if (
        ...
        or not _inputs_not_newer(inputs, panel)
...
def _read_state_partition(
    ...
    *,
    ...
    if not _inputs_not_newer(inputs, path):
        raise ValueError(f"canonical {family} partition is older than or missing a source input: {path}")
