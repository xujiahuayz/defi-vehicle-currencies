...
def reserve_delta(row: dict) -> tuple[Decimal, Decimal]:
    ...
    values = Decimal(str(row["amount0_delta"])), Decimal(str(row["amount1_delta"]))
    if any(not value.is_finite() for value in values):
        raise InvalidOperation("non-finite reserve delta")
    return values
    ...
    *,
    ...
    for row in snapshots.to_dict("records"):
        ...
        if (
            ...
            or not token0
            or not token1
            or any(not value.is_finite() or value <= 0 for value in state)
        ):
            continue
    ...
    for venue in venues:
        ...
        for row in day_frame[day_frame["record_type"].eq("liquidity")].to_dict("records"):
            ...
            try:
                ...
                continue
            if pool and all(value.is_finite() for value in delta):
                hour = timestamp - timestamp % 3600
